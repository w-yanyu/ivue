<template>
  <!-- 规则树  主要用在规则树定义 -->
  <!-- G6 决策树渲染块 -->
  <div id="container_view" @contextmenu.prevent></div>
</template>
...
<script>
 ...
 `);
  export default {
    ...
    name: 'DecisionTreeViews',

    data() {
      return {
        cParams: this.cParentParams,
        graph: null, //G6实例
        container_view: '', //G6的画布
        width: '', //画布宽高
        height: ' ', //画布宽高
    ...
    methods: {
      ...
      init() {
        ...
        //获取容器元素
        that.container_view = document.getElementById('container_view');
        that.width = that.container_view.scrollWidth;
        console.log(`that.container_view`, that.container_view);
        console.log('this.setHeight----------------',this.setHeight);
        that.height =this.setHeight==='0'? window.innerHeight || that.container_view.scrollHeight:this.setHeight;
          ...
          draw(cfg, group) {
            ...
            const treeNum = group.addShape('text', {
              attrs: {
                ...
                x: 12 + nodeOrigin.x,
                y: 23 + nodeOrigin.y,
                text: nodeType === 3 ? '根节点' : nodeType === 2 ? '叶子节点' : '分支节点',
                fontSize: 16,
      ...
      initGraph(data) {
        ...
        const tooltip = new G6.Tooltip({
          // offsetX and offsetY include the padding of the parent container_view
          offsetX: 20,
          offsetY: 30,
