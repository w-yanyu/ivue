<template>
  <!-- 规则树  主要用在规则树定义 -->
  <!-- G6 决策树渲染块 -->
  <div id="containerAfter" @contextmenu.prevent></div>
</template>
 ...
 `);
  export default {
    ...
    methods: {
     ...
     //获取容器元素
     that.container = document.getElementById('containerAfter');
     that.width = that.container.scrollWidth || window.innerWidth;
          ...
          draw(cfg, group) {
            ...
            if (ruleDesc) {
              ...
              if (that.byteLength(ruleDesc) > 36 && that.byteLength(ruleDesc) <= 72) {
                ...
                //let ruleDescEnd = ruleDesc.length > 24 ? ruleDescOther.substr(0, 16) + '...' : ruleDescOther
                //console.log('ruleDescEnd', ruleDescEnd);
                group.addShape('text', {
                  attrs: {
                    ...textConfig,
                    x: 12 + nodeOrigin.x,
