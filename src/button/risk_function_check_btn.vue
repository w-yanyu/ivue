...
<script>
  ...
  export default {
    ...
    mounted() {
      console.log('risk_function_check_btn----cParentParams', this.cParentParams);
    },
    beforeDestroy() {},
    methods: {
      handleClick() {
        ...
        if (this.$dataBus.getCurrentScope(this).datagrid.length > 0) {
          ...
          for (const child of temp) {
            let obj = {
              ...
              paramsValues: child.paramsValues,
              sort: child.row_id
            }
            tableArry.push(obj)
          }
        ...
        that.$http.invokeAPI(request_url, "post", request_map).then(response => {
          ...
          if (response.code === "200") {
            ...
            this.$message({
              type: "success",
              message: "校验函数成功,"+"返回值:" +response.data.message
            });
           // that.$dataBus.doCallBack(that, response);
          } else {
            this.$message({
              message: "校验函数失败,"+"错误信息:" +response.data.message,
              type: 'error'
            });
          }
        });
      ...
      isArrayCompleted(arrayData) {
        ...
        for (let i in arrayData) {
          if (arrayData[i].paramsDataType === '') {
            that.$message.error(`校验参数表格 第${i + 1}行 参数类型未选择`);
            isCompleted = false;
            break;
          } else if (arrayData[i].paramsCode === '') {
            that.$message.error(`校验参数表格 第${i + 1}行 参数字段未填写`);
            isCompleted = false;
            break;
          } else if (arrayData[i].paramsDesc === '') {
            that.$message.error(`校验参数表格 第${i + 1}行 参数描述未填写`);
            isCompleted = false;
            break;
          }
          else if (arrayData[i].paramsValues === '') {
            that.$message.error(`校验参数表格 第${i + 1}行 参数值未填写`);
            isCompleted = false;
            break;
          }
        }
