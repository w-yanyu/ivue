    ...
    <el-table :data="newList" :row-class-name="tableRowClassName" border stripe style="width: 100%">
      ...
      <el-table-column  width="200px"  :label="$i18ns('附件查看')">
        <template slot-scope="scope">
          ...
          <el-button size="small" type="text" @click="deleteButton(scope.$index)" v-if="tranType!='detail'&&scope.row.file_path!=''&&scope.row.file_path!=null">{{$i18ns('删除')}}</el-button>

        </template>
      </el-table-column>
      <el-table-column width="200px" :label="$i18ns('附件上传')" v-if="tranType!='detail'">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    initFileList(){
      ...
      let params = {
        servicecode: "pt0323",
        data_list_no: that.cParentParams.data_list_no,
        part_project_no:that.part_project_no,
    ...
    initFileList2(){
      ...
      let params = {
        servicecode: "ar0161",
        doc_check_list_no: that.cParentParams.data_list_no
      }
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "POST", params)
        .then(res => {
          if (res && res.code === "200") {
