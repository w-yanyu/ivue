...
export default {
  ...
  created() {
    ...
    console.log("sdsds",this.cParentParams);
    this.getTabs();
  },
  methods: {
    /**
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(res => {
        if (res && res.code === "200") {
          ...
          res.data.agrelist.forEach((item1, index1) => {
            //将协议加入步骤条
            that.pageList.push({ parts_name: item1.mould_name, parts_no: item1.protocol_template_no, protocol_template_version: item1.version, info_url: "@agreeBaseInfoAdd",add_url:"@agreeBaseInfoAdd",update_url:"@agreeBaseInfoAdd"})

          });
          if(that.cParentParams.data_list_no!=''&&that.cParentParams.data_list_no!=null&&that.cParentParams.data_list_no!=undefined){
            //加入文件上传部件
            that.pageList.push({ parts_name: "附件上传", parts_no: "999", info_url: "@projfileModify",add_url:"@projfileUpload",update_url:"@projfileModify"})
          }
        }
