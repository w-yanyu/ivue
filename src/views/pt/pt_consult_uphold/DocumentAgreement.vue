...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        part_no: [
          { required: true, message: "请输入合作商编号" }
        ],
        part_protocol_no: [
  ...
  methods: {
    ...
    fileInit() {
      let params = {
        part_no: this.cParentParams.part_no,
        servicecode: "qrfile",
      }
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(res => {
        if (res && res.code === "200") {
          res.data.forEach(item => {
            ...
            this.ruleForm.fileList.push({ file_name: item.filena, file_path: item.filepa, original_file_name: item.initna });
          });
        }
      }).catch(err => {
        console.error(err);
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          let params = {
            ...
            list01: this.ruleForm.fileList,
            // file_name: this.ruleForm.fileList.file_name,
            // original_file_name: this.ruleForm.fileList.initName
