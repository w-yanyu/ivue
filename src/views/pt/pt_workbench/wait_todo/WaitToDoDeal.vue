  ...
  <div>
    <div class="todo-top">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="240px" class="custom-common--form">
        <el-row>
          ...
          <el-col :span="8" v-if="routidSelect">
            <el-form-item label="下一处理人:" prop="user_id" :rules="[{ required: true, message: '请选择下一处理人' }]">
              <el-select v-model="ruleForm.user_id" multiple class="filter-item" placeholder="请选择下一处理人" size="small">
        ...
        <el-row>
          <el-col :span="20">
            <el-form-item label="审批意见:" prop="appr_desc" :rules="[{required: true, message: '请输入审批意见'}]">
              <el-input
                v-model="ruleForm.appr_desc"
                type="textarea"
                :rows="4"
                resize="none"
              ></el-input>
...
export default {
  ...
  created() {
    ...
    this.c_$params.appl_date = this.cParentParams.appl_date;//申请日期
    ...
    // 进件中心查产品申请类型
    if(this.cParentParams.appId === 'ptOnl'){
      this.getApplyType();
    }
  },
  methods: {
