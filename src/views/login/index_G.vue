<template>
  <div class="login-container" v-if="loginShow">
    ...
    <div class="center-div">
      <div class="withd-flex1">
        <div class="withd-flex1-box">
          <p class="withd-flex1-p">
            {{ $i18ns(loginWel || "welcome") }}
            <img :src="welImg" class="withd-flex1-img" />
          </p>

          <p class="withd-flex1-title">{{ title }}</p>
        </div>
        <div class="withd-flex1-box withd-flex1-boxb">
          <p class="withd-flex1-pb">{{ $i18ns(loginTip || "Please Login") }}</p>
        </div>
      </div>
      <div class="withd-flex2 ">
        <el-form
          autocomplete="on"
          :model="loginForm"
          :rules="loginRules"
        ...
        >
          ...
          <!-- 后期考虑中英文位置切换灵活选择,暂时注释 -->
          <el-form-item class="no_bg_bottom" prop="ui_language" v-if="false">
            <el-row class="lanBlock">
              <el-col>
...
import loginBackground from "../../../public/login/login_G_background.png";
import welImg from "../../../public/login/login_G_img.png";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      welImg: welImg, // 欢迎登录小图标
      loginLogo: loginLogo, // logo图标
      title: "", // 标题
      loginWel: "loginWel", // 欢迎登陆提醒
      loginTip: "loginTip", // 请先登录提醒
      footLabel: "", // 末尾文字
      position: "", // 展示位置
      visible: false,
