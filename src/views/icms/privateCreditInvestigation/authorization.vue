<template>
  <div style="width: 100%; text-align: center" id="print">
    <div ref="myElement" class="con">
      <div class="water">
        <div ref="myRow" v-for="(row, rowIndex) in rowsH" :key="rowIndex" class="water-row">
          <div v-for="(text, colIndex) in 3" :key="colIndex" class="water-cell">{{ userCode }} <br> {{ branch_name }} <br>
            {{ reportHeaderDate }}</div>
        </div>
      </div>
      <div>
        <el-image class="icon-image" :src="require('@/views/icms/creditReport/征信报告图片.png')" fit="contain">
        </el-image>
        <h1>个人征信报告 <br />（银行版）</h1>
      </div>
      <!-- <h1>个人信用报告</h1>
      ...
      <AuthorizationTableItem v-if="isReportHeaderShow" :leftFlex="3" :rightFlex="2" :leftText="'报告编号：' + reportHeaderNo"
      ...
      <FraudHeadItem style="margin-top: 20px;" v-if="isFraudHeaderShow" :leftFlex="3" :rightFlex="2"
        :leftText="'申请主体申请设置防欺诈要求。联系电话“' + anti_fraud_tel + '”。防欺诈要求添加于' + anti_fraud_start_date + '，截止日期' + anti_fraud_end_date + '。'" />
      <el-table class="tableMargin" v-if="isFraudHeaderShow" :data="ArbCReportheader" border style="width: 100%">
        <el-table-column prop="anti_fraud_start_date" align="center" label="生效日期">
        </el-table-column>
        <el-table-column prop="anti_fraud_end_date" align="center" label="截止日期">
        </el-table-column>
      </el-table>
      ...
      <el-table class="tableMargin" :data="ArbCSpouse" style="width:100%; margin-top:5px !important;" border>
        ...
        <el-table-column label="婚姻状况" prop="marital_status" :formatter="formatMaritalStatus"
        ...
        <el-table-column label="就业状况" prop="employment_status" :formatter="formatEmploymentStatus"
          align="center"></el-table-column>
      ...
      <el-table class="tableMargin" :data="ArbCSpouse" style="width:100%; margin-top:5px !important;" border>
        ...
        <el-table-column label="证件类型" prop="sps_cert_type" :formatter="formatSpsCertType"
          align="center"></el-table-column>
      ...
      <el-table class="tableMargin" :data="ArbCProfessional" style="width:100%; margin-top:5px !important;" border>
        ...
        <el-table-column label="单位性质" prop="corp_property" :formatter="formatCorpProperty"
          align="center"></el-table-column>
      ...
      <AuthorizationTable1 style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px;" class="tableMargin"
        :tableData="ArbCSummaryCue" />

      <h4>（三）信贷交易违约信息概要</h4>

      <el-table class="tableMargin" :show-summary="true" :data="ArbCFellbackSum"
        style="width:100%; margin-top:5px !important;" border>
      ...
      <el-table class="tableMargin" :data="ArbCNoncreditInfo" style="width:100%; margin-top:5px !important;" border>
        <el-table-column label="后付费业务欠费信息总汇" align="center">
          <el-table-column label="业务类型" prop="paid_busi_type" :formatter="formatPaidBusiType"
            align="center"></el-table-column>
      ...
      <el-table class="tableMargin" :data="ArbCPublicInfo" style="width:100%; margin-top:5px !important;" border>
        <el-table-column label="公共信息汇总" align="center">
          <el-table-column label="信息类型" prop="comm_info_type" :formatter="formatCommInfoType"
            align="center"></el-table-column>
      ...
      <el-table class="tableMargin" :data="ArbCQuerySummary" style="width:100%; margin-top:5px !important;" border>
        <el-table-column label="上一次查询记录" align="center">
          ...
          <el-table-column label="上一次查询机构" prop="last_query_org_type" :formatter="formatOrgType"
            align="center"></el-table-column>
          <el-table-column label="上一次查询原因" prop="query_reason" :formatter="formatSqueryReason"
            align="center"></el-table-column>
        </el-table-column>
      </el-table>
      ...
      <!-- TODO -->
      <el-table class="tableMargin" :data="xxx" v-if="isYyInfoShow" style="width:100%; margin-top:25px !important;"
        border>
      ...
      <h3>三、信贷交易信息明细</h3>
      <h4>（一）被追偿信息</h4>
      <CollAccountDeatils style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :collAccountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'sys_flg': this.sysflg }" />

      <h4>（二）非循环贷账户</h4>
      <AccountDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :accountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'account_type': 'D1', 'sys_flg': this.sysflg }" />

      <h4>（三）循环额度下分账户</h4>
      <AccountDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :accountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'account_type': 'R4', 'sys_flg': this.sysflg }" />

      <h4>（四）循环贷账户</h4>
      <AccountDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :accountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'account_type': 'R1', 'sys_flg': this.sysflg }" />

      <h4>（五）贷记卡账户</h4>
      <CardAccountDeatils style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :cardAccountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'account_type': 'R2', 'sys_flg': this.sysflg }" />

      <h4>（六）准贷记卡账户</h4>
      <CardAccountDeatils style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 0px; "
        :cardAccountParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'account_type': 'R3', 'sys_flg': this.sysflg }" />


      <h4>（七）相关还款责任信息</h4>
      <h4>有相关还款责任的个人借款</h4>
      <GuaranteeDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px; "
        :guaranteeParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'cert_type': '1', 'sys_flg': this.sysflg }" />

      <h4>有相关还款责任的企业借款</h4>
      <GuaranteeDetailsQy style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px; "
        :guaranteeParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'cert_type': '2', 'sys_flg': this.sysflg }" />

      <h4>（八）授信协议信息</h4>
      <CreditDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px; "
        :creditParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'sys_flg': this.sysflg }" />


      <h3>四、非信贷交易明细</h3>
      <h4>后付费记录</h4>
      <NontradeDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px; "
        :nontradeParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'sys_flg': this.sysflg }" />
      ...
      <h4>（五）住房公积金参缴记录</h4>
      <FundDetails style="box-sizing: content-box; width: 100%; margin: 5px 0px 0px 12px; "
        :fundParam="{ 'apply_no': this.applyno, 'credit_reports_no': this.creditno, 'sys_flg': this.sysflg }" />


      <h4>（六）低保救助记录</h4>
      <el-table class="tableMargin" :data="ArbCSalvation" style="width:100%; margin-top:5px !important;" border>
        <el-table-column label="编号" type="index" align="center"></el-table-column>
        <el-table-column label="人员类别" prop="personnel_type" :formatter="formatPersonnelType"
          align="center"></el-table-column>
      ...
      <el-table class="tableMargin" :data="ArbCRecordDetail" style="width:100%; margin-top:5px !important;" border>
        ...
        <el-table-column label="查询机构" prop="query_org" align="center"></el-table-column>
        <el-table-column label="查询原因" prop="query_reason" :formatter="formatQueryReasons"
          align="center"></el-table-column>
      </el-table>
      <h3>报告说明</h3>
...
export default {
  ...
  created() {
    ...
    this.branch_name = JSON.parse(localStorage.getItem('user_info')).branch_name;
    this.userCode = JSON.parse(localStorage.getItem('user_info')).userCode;
  },
