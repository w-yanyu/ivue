<template>
  <div class="guarantee_details_container">
    <GuaranteeDetailItem
...
<script>
import GuaranteeDetailItem from "./GuaranteeDetailItem.vue";
import MyAxios from "pte-ui/utils/MyAxios";
/**
 * 所有相关还款责任信息展示组件
 * TODO 需要在这里处理过滤相关还款责任，因为涉及到相关还款责任序号的显示，最好提前控制。如果不在乎序号就可以直接传入。
 * 每个GuaranteeDetailItem负责具体的相关还款责任显示，相关还款责任显示具体逻辑在该组件中修改。
 */
export default {
  ...
  components: { GuaranteeDetailItem },
  data() {
    return {
      details: [],
      // 字典集合
  ...
  async created() {
    ...
    this.initData(this.guaranteeParam);
  },
  methods: {
    async initData(form) {
    ...
    async getDictLists() {
      return Promise.all([
        this.getDictList('E_C_BORROWERTYPE'),//借款人身份类别
        this.getDictList('E_C_REPAYDUTYPERSIONTYPE'),//相关还款责任人类型
        this.getDictList('AR_E_ORG_TYPE'),//机构类型
        this.getDictList('E_C_LOANTYPE'),//个人业务种类
        this.getDictList('E_C_CLASS5STATE'),//五级分类 
        this.getDictList('E_CURRENCY'),//币种
        this.getDictList('E_QUERY_ORG_TYPE'),
        this.getDictList('E_GUARANTEE_FORMAT'),//企业业务种类
