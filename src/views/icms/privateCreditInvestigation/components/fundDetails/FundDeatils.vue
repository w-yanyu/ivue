<template>
  <div class="fund_details_container">
    <FundDetailItem
...
<script>
import FundDetailItem from "./FundDetailItem.vue";
import MyAxios from "pte-ui/utils/MyAxios";
/**
 * 所有相关还款责任信息展示组件
 * TODO 需要在这里处理过滤相关还款责任，因为涉及到相关还款责任序号的显示，最好提前控制。如果不在乎序号就可以直接传入。
 * 每个FundDetailItem负责具体的相关还款责任显示，相关还款责任显示具体逻辑在该组件中修改。
 */
export default {
  name: "FundDetails",
  props: {
  ...
  methods: {
    ...
    async getDictLists() {
      return Promise.all([
        this.getDictList('E_C_ACCFUNDSTATE'),//缴费状态
      ])
    }
