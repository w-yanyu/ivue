...
export default {
  ...
  methods: {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then((res) => {
          if (res && res.code === "200") {
            ...
            let showList = [];
            this.getLoanDetails(this.cParentParams,(data)=>{
                this.c_$params.lend_no = data.lend_no;
                for(let i=0;i<reqList.length;i++){
                  let node = new Object;
                  node.tree_seq = reqList[i].tree_seq;  
                  node.tree_menu_name = reqList[i].tree_menu_name;
                  if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000019" && data.farm_flag == this.is_NO) {
                    node.tabStatus = false;
                  }
                  else if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000021" && data.is_pbc == this.is_NO) {
                    node.tabStatus = false;
                  }
                  else if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000020" && data.realty_flag == this.is_NO) {
                    node.tabStatus = false;
                  }
                  else if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000022" && data.proj_loan_flag == this.is_NO) {
                    node.tabStatus = false;
                  }
                  else if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000023" && data.repay_type ==="8") {
                    node.tabStatus = true;
                    
                  }
                  else if (reqList[i].frame_cd === "loanIOU" && reqList[i].func_cd === "110000018" && data.repay_type ==="8") {
                    node.tabStatus = false;
                  }
                  else {
                    node.tabStatus = true;
     ...
     * 获取放款明细信息
     */
    getLoanDetails(params,callback) {
      let condition = {
    ...
    checkModifyOrInfo(pageType) {
      if (pageType === "info") {
        for (let i = 0; i < this.tabList.length; i++) {
          //借据详情页替换
          if (this.tabList[i].toRequest.url === "@LendIOUDetails") {
            this.tabList[i].toRequest.url = "@LendIOUDetails_info";
          }
        }
      }
    },
