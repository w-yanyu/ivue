<template>
  <div>
    <el-button type="primary" @click="onSubmit" class="tableSubmit">
      保存
    </el-button>
    <el-table
      ref="multipleTable"
      ...
      >
        <template slot-scope="scope">
          <el-input v-model="scope.row.busi_assign_amt" @blur="changeAmt(scope.row)"></el-input>
        </template>
      </el-table-column>
      <el-table-column
    ...
    onSubmit() {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(res => {
        if (res && res.code === "200") {
          ...
          //刷新父级页面
          // that.$dataBus.doParentCallBack(this, res);
          that.$dataBus.doCallBack(that.$parent, res);
          // that.$dialog.close();
        }
      }).catch(err => {
        console.error(err);
