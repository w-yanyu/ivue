    ...
    <div class="menu-form--container">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="130px" class="custom-common--form">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item :label="$i18ns('担保类型')" prop="guar_modality" label-width="203px">
              <el-select v-model="ruleForm.guar_modality" :placeholder="$i18ns('请选择担保类型')" @change="changeGuarTypeValue" :disabled=fieldSwitch>
                <el-option v-for="item in guar_modalityOptions" :key="item.dictId" :label="item.dictName"
                  :value="item.dictId">
                </el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item :label="$i18ns('币种')" prop="ccy_code " label-width="203px">
              <el-select v-model="ruleForm.ccy_code"  :placeholder="$i18ns('请选择币种')" @change="changeStatusValue" :disabled=fieldSwitch>
          ...
          <el-col :span="12">
            <el-form-item  :label="$i18ns('保证合同金额')" prop="contract_amt" label-width="203px">
              <el-input class="input_el" disabled="true" v-model.number="ruleForm.contract_amt" @blur="guarAmtInput($event)" type="number">
              </el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item  :label="$i18ns('合同金额(折算基准币种)')" prop="contract_base_amt" label-width="203px">
              <el-input class="input_el" disabled="true" v-model.number="ruleForm.contract_base_amt" @blur="guarBaseAmtInput($event)" type="number">
              </el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item  :label="$i18ns('本次使用金额')"  prop="this_use_amt" label-width="203px">
              <el-input class="input_el" v-model.number="ruleForm.this_use_amt" placeholder="请输入本次使用金额"
          ...
          <el-col :span="12">
            <el-form-item :label="$i18ns('合同签订日期')"  prop="sign_date" label-width="203px">
              <el-date-picker v-model="ruleForm.sign_date" :disabled=false type="date" placeholder="选择日期" format="yyyyMMdd" value-format="yyyyMMdd">
              </el-date-picker>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item  :label="$i18ns('合同起始日期')" prop="guar_start_date" label-width="203px">
              ...
              <el-date-picker  class="int" v-model="ruleForm.guar_start_date" :picker-options="startDatePicker"
                type="date" :placeholder="$i18ns('起始日期')" format="yyyyMMdd" value-format="yyyyMMdd" style="width: 100%" :disabled=false>
              </el-date-picker>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item :label="$i18ns('合同到期日期')" prop="guar_end_date" label-width="203px">
              ...
              <el-date-picker  class="int" v-model="ruleForm.guar_end_date" :picker-options="endDatePicker"
                type="date" :placeholder="$i18ns('结束日期')" format="yyyyMMdd" value-format="yyyyMMdd" style="width: 100%" :disabled=false>
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <div class="divTitle"><span class="required">*</span>{{$i18ns('关联保证人')}}</div>
          ...
          <el-table ref="multipleTable" :data="tableData" highlight-current-row tooltip-effect="dark"
            ...
            <el-table-column prop="appt_no" :label="$i18ns('保证人编号')" align="center">
              <!-- <template slot-scope="scope">{{ scope.row.appt_no }}</template> -->
            </el-table-column>
            <el-table-column prop="cust_name" :label="$i18ns('保证人名称')" align="center">
            </el-table-column>
            <el-table-column prop="guar_amt" :label="$i18ns('保证金额')" align="center">
            </el-table-column>
            <el-table-column prop="guar_ccy" :label="$i18ns('币种')" align="center" :formatter="guarCcyFormat">
            </el-table-column>
          </el-table>
        </el-row>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          guar_rule: [{
            required: true,
            message: this.$i18ns('请选择担保规则'),
            trigger: "blur",
          }, ],
          guar_modality: [{
            required: true,
            message: this.$i18ns('请选择担保类型'),
            trigger: "blur",
          }, ],
          contract_amt: [{
            required: true,
            message: this.$i18ns('请输入保证金金额'),
            trigger: 'blur',
          }],
          this_use_amt: [{
              required: true,
              message: this.$i18ns('请输入本次使用金额'),
              trigger: 'blur',
            }
          ],
          sign_date: [{
            required: true,
            message: this.$i18ns('请输入签订日期'),
            trigger: 'blur'
          }],
           guar_start_date: [{
            required: true,
            message: this.$i18ns('请输入起始日期'),
            trigger: 'blur'
          }],
           guar_end_date: [{
            required: true,
            message: this.$i18ns('请输入结束日期'),
            trigger: 'blur'
          }],
        },
        tableData: [],
    ...
    mounted() {
      ...
      that.initGuarTypeOptions();//加载担保类型
    ...
    methods: {
      initDefineData() {
        ...
        that.ruleForm.guar_modality = that.cParentParams.guar_modality; //担保类型
        that.ruleForm.ccy_code = "156"; //币种
      ...
      initGuarTypeOptions() {
        let request_map = {};
        let request_url = "/SUMP/rest/dict";
        request_map["dictType"] = "AR_E_GUAR_CATG";
        request_map["dictKey"] = "AR_E_GUAR_CATG";
        this.$http
          .invokeAPI(request_url, "post", request_map)
          .then((response) => {
            if (response.code === "200") {
              this.guar_modalityOptions = response.data;
            } else {
              MyMessage({
                message: response.message,
                type: "error",
      ...
      initData() {
        ...
        request_map["servicecode"] = "ar0725";
        request_map["apply_no"] = that.cParentParams.apply_no;
        request_map["query_type"] = "1";
        request_map["guar_contract_no"] = that.cParentParams.guar_contract_no;
        this.$http
          .invokeAPI(request_url, "post", request_map)
          .then((response) => {
      ...
      clickConfirm() {
        ...
        request_map["servicecode"] = "ar0322";
        request_map["apply_no"] = that.cParentParams.apply_no;
        request_map["guar_rule"] = that.ruleForm.guar_rule;
        request_map["guar_modality"] = that.ruleForm.guar_modality;
        request_map["ccy_code"] = that.ruleForm.ccy_code;
        request_map["exchange_rate"] = 1;
        ...
        request_map["guarantorInfo"] = that.multipleSelection;
        request_map["guar_contract_no"] = that.cParentParams.guar_contract_no;
        that.$http
          .invokeAPI(request_url, "post", request_map)
          .then((response) => {
      ...
      delData() {
        ...
        that.ruleForm.guar_modality = ""; //担保类型
