                        ...
                        <el-col :span="8">
                            <el-form-item label="业务类别" prop="busi_type">
                                <el-select v-model="payListSelForm.busi_type"   clearable disabled>
                                    <el-option
                                    v-for="(item, index) in busi_typeList"
                                    :key="index"
                                    :label="item.dictName"
                                    :value="item.dictId"
                                    ></el-option>
                        ...
                        <el-col :span="8">
                            <el-form-item label="额度类别" prop="quota_type">
                                <el-select v-model="payListSelForm.quota_type"   clearable disabled>
                                    <el-option
                                    v-for="(item, index) in quota_typeList"
                                    :key="index"
                                    :label="item.dictName"
                                    :value="item.dictId"
                                    ></el-option>
                        ...
                        <el-col :span="8">
                            <el-form-item label="额度状态" prop="quota_status">
                                <el-select v-model="payListSelForm.quota_status" disabled  clearable>
                                    <el-option
                                    v-for="(item, index) in quota_statusList"
                                    :key="index"
        ...
        */
            ...
            MyAxios.invokeAPI("/SUMP/rest/dict", "post", params).then(res => {
                ...
                this[listKey] = res.data;
                  ...
                  this.StatusList.map((item,index)=> {
                      if (item.sortNo == 1 || item.sortNo == 4) {
                          if (item.dictName == "有效") {
                              item.dictName = "解冻"
                          }
                          item.disabled = false;
        ...
        submit(){
            ...
            this.$confirm("是否提交额度冻结解冻申请?", "提示", {
                ...
                let reqMap = {
                    servicecode: "ar1300",
                    cust_no:this.payListSelForm.cust_no,
                    cust_name:this.payListSelForm.cust_name,
                    limit_no:this.payListSelForm.limit_no,
                    busi_no:this.payListSelForm.cl_type_cd,
                    ccy_code:this.payListSelForm.ccy_code,
                    cl_amt:this.payListSelForm.cl_amt,
                    avail_amt:this.payListSelForm.avail_amt,
                    cl_level:this.payListSelForm.cl_level,
                    cycle_flag:this.payListSelForm.cycle_flag,
                    effect_date:this.payListSelForm.effect_date,
                    expiry_date:this.payListSelForm.expiry_date,
                    status:this.payListSelForm.status,
                }
