    ...
    <el-form
      ...
      :span="24">
        <el-form-item class="submit-item">
            <el-button size="small" type="primary" class="submit-item--btn" @click="submit">启动流程</el-button>
        </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  name: "ColsOrganizeEditSubmitBtn",
  props: {
    cMeta: Object, // 布局相关数据
    ...
    submit() {
      this.$refs.ruleForm.validate( valid => {
        if(valid) {
          ...
          (params.trantp = "UPDATE"),
          params.custna = this.cParentScope.parentFormModel.organize_name;
          params.servicecode = "co4009";
          params.is_start = "Y";
          let that = this;
             if (params.organize_status != 2) {
            this.$message({
              type: "warning",
              message: "只有生效记录才能进行修改操作",
            });
          } else {
          MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(res => {
            if(res && res.code === "200"){
              ...
              this.$publicFn.openNavTab("@colWaitToDoDeal", "待办事项办理", params, this)
            }
          }).catch(err => {
            console.error(err);
          })}
