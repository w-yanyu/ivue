        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="借据号" prop="loan_no">
              <el-input v-model="basicForm.loan_no" @blur="customerAccountBlur" ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-collapse-item name="1">
          ...
          <el-form ref="applyForm" :rules="applyRules" :model="applyForm" label-width="200px" class="custom-common--form">
            ...
            <el-row class="ticket-detail" v-for="(item, index) in applyForm.list" :key="index">
              <el-row>
                <el-col :span="12">
                  <el-form-item label="凭证类型" :prop="`list[${index}].vchr_tp`" v-show="false">
                    <el-select v-model="applyForm.list[index].vchr_tp" placeholder="请选择凭证类型">
                      <el-option
                ...
                <el-col :span="12">
                  <el-form-item label="时间凭证区间" :prop="`list[${index}].ticketInterval`">
                    <el-date-picker
                     v-model="applyForm.list[index].ticketInterval"
                     type="daterange"
                     range-separator="至"
                ...
                <el-col :span="12">
                  <el-form-item label="总金额" :prop="`list[${index}].vchr_amt`">
                    <el-input v-model="applyForm.list[index].vchr_amt"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                   <el-form-item  label="附件" :prop="`list[${index}].atchmt_file`">
                    <el-upload
                      :on-change="(file, fileList) => { accessoryChange(file, fileList, index) }"
                      :on-remove="(file, fileList) => { accessoryRemove(file, fileList, index) }"
                      :limit="5"
                      :file-list="applyForm.list[index].ticketAccessoryList"
                      :auto-upload="false"
                      :on-preview="uploadPreview"
                    >
                      <el-button type="primary" size="small" v-show="applyForm.list[index].ticketAccessoryList.length < 5">{{$i18ns('选择')}}文件</el-button>
                    </el-upload>
                  </el-form-item>
                </el-col>
              </el-row> 
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item  label="款项用途" prop="loan_purp">
                  <el-input v-model="applyForm.loan_purp" @blur="applyuseurl"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item :label="$i18ns('备注')">
                  <el-input type="textarea" resize="none" :rows="4" v-model="applyForm.vou_remark"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
      ...
      // formData.append("org_id", userInfo.org_id);
        ...
        .then(res => {
          if (res && res.code === "200" && res.data && res.data.path) {
            ...
            file.url = window.URL.createObjectURL(file.raw);
            file.type = file.name.substring(file.name.lastIndexOf(".") + 1, file.name.length);
            this.applyForm.creden_list.push({"file_path":res.data.path,"orig_file_nm":file.name}); // 记录上传成功文件的文件路径
            if (fileList.length === 1) {
