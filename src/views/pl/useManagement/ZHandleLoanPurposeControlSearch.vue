<template>
  <div>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="借据号" prop="customerAccount">
              <el-input
                v-model="basicForm.customerAccount"
                :disabled="true"
              ></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="产品名称" prop="productChineseName">
              <el-input
                v-model="basicForm.productChineseName"
                :disabled="true"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="贷款形态">
              ...
              <el-select
                v-model="basicForm.accountStatus"
                prop="accountStatus"
                :disabled="true"
              >
                <el-option
            ...
            >
              ...
              >
                <el-button
                  size="small"
                  type="primary"
                  v-show="htfjFileList.length < 5"
                  >{{$i18ns('选择')}}文件</el-button
                >
              </el-upload>
            </el-form-item>
          </el-col>
          ...
          >
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="放款距申请时间差">
                  <el-input
                    v-model="applyForm.applyTimeDiff"
                    :disabled="true"
                  ></el-input>
                </el-form-item>
              </el-col>
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item
                  label="证明材料"
                  :class="{ 'require-asterisk': applyForm.isSelf === '0' }"
                  prop="credentialList"
                >
                  <el-upload
                    class="credential-upload"
                  ...
                  >
                    <el-button
                      size="small"
                      type="primary"
                      v-show="credentialFileList.length < 5"
                      >{{$i18ns('选择')}}文件</el-button
                    >
                  </el-upload>
                </el-form-item>
            ...
            <el-row
              class="ticket-detail"
              v-for="(item, index) in applyForm.ticketList"
              :key="index"
            >
              <el-row>
                <el-col :span="12">
                  <el-form-item
                    label="凭证类型"
                    :prop="`ticketList[${index}].ticketType`"
                  >
                    <el-select
                      v-model="applyForm.ticketList[index].ticketType"
                      placeholder="请选择凭证类型"
                      :disabled="true"
                    >
                      <el-option
                ...
                <el-col :span="12">
                  <el-form-item
                    label="时间凭证区间"
                    :prop="`ticketList[${index}].ticketInterval`"
                  >
                    <el-date-picker
                ...
                <el-col :span="12">
                  <el-form-item
                    label="总金额"
                    :prop="`ticketList[${index}].ticketSum`"
                  >
                    <el-input
                      v-model="applyForm.ticketList[index].ticketSum"
                      :disabled="true"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item
                    class="require-asterisk"
                    label="附件"
                    :prop="`ticketList[${index}].ticketAccessory`"
                  >
                    <el-upload
                      :on-preview="uploadPreview"
                      :on-change="
                      ...
                      :limit="5"
                      :file-list="
                        applyForm.ticketList[index].ticketAccessoryList
                      "
                      :auto-upload="false"
                      :disabled="true"
                    >
          ...
          >
            <el-row>
              <el-col :span="12">
                <el-form-item
                  label="贷款用途凭证交易对手方"
                  prop="voucherAnalogue"
                >
                  <el-input
                    v-model="voucherForm.voucherAnalogue"
                    @blur="voucherAnalogueBlur"
                    :disabled="true"
                  ></el-input>
                </el-form-item>
              </el-col>
              ...
              <el-col :span="24">
                <el-form-item :label="$i18ns('备注')">
                  <el-input
                    ...
                    resize="none"
                    :rows="4"
                    v-model="voucherForm.voucherRemark"
                    :disabled="true"
                  ></el-input>
                </el-form-item>
              </el-col>
                ...
                >
                  ...
                  >
                    <el-button
                      size="small"
                      type="primary"
                      v-show="aaumUploadFileList.length < 5"
                      >{{$i18ns('选择')}}文件</el-button
                    >
                  </el-upload>
                </el-form-item>
    ...
    <image-preview
      ...
      @close="closePreview"
    />
    <el-dialog :visible.sync="pdfVisible" title="PDF文件预览" width="1200px" :append-to-body="true">
...
import {
  ...
  voucherRules,
  validateHandle,
} from "../derate/loanPostConstant";
import _ from "lodash";
import ImagePreview from "COM/ImagePreview";

export default {
  name: "ZHandleLoanPurposeControlSearch",
  props: {
    ...
    ticketTypeListFilter() {
      ...
      for (let i = 0; i < this.applyForm.ticketList.length; i++) {
        for (let j = 0; j < ticketTypeList.length; j++) {
          if (
            this.applyForm.ticketList[i].ticketType === ticketTypeList[j].dictId
          ) {
  ...
  created() {
    ...
    this.getVoucherInfo();
    this.applyRules.credentialList.push({
      validator: this.credentialValidate,
      trigger: "blur",
    });
    this.basicRules.htfjUploadList.push({
      validator: this.htfjValidate,
      trigger: "blur",
    });
    this.voucherRules.aaumUploadList.push({
      validator: this.aaumUploadValidate,
      trigger: "blur",
    });
    this.voucherRules.zuozhengUploadList.push({
      validator: this.zuozhengUploadValidate,
      trigger: "blur",
    });
  ...
  methods: {
    ...
    accessoryValidate(rule, value, callback) {
      ...
      console.log(
        this.applyForm.ticketList[index].ticketAccessoryList,
        111111111
      );
      ...
      this.$http
        ...
        .then((res) => {
          if (res && res.code === "200" && res.data) {
            ...
            this.basicForm.loanLimit = response.loan_amt; //crdtam
            ...
            this.voucherForm.voucherAnalogue = response.cou_parties; //交易对手方
            this.applyForm.isSelf = response.ifself; //是否本人
            this.voucherForm.isSame = response.is_match; //对手方是否一致
            this.voucherForm.voucherRemark = response.cou_remark; //对手方备注 cou_remark
          }
        });
    },
    /**
      ...
      this.$http
        ...
        .then((res) => {
          if (res && res.code === "200" && res.data) {
            ...
            if (response.zmcl.length > 0) {
              for (let j = 0; j < response.zmcl.length; j++) {
                const zmcl = response.zmcl[j];
              ...
              console.log(this.credentialFileList)
            }

            if (response.gslxx.length > 0) {
              for (let j = 0; j < response.gslxx.length; j++) {
              const gslxx = response.gslxx[j];
      ...
      this.$http
        ...
        .then((res) => {
          if (res && res.code === "200" && res.data) {
            ...
            console.log(ticketList, "voucher");
            for (let i = 0; i < ticketList.length; i++) {
              const ele = {
                ticketType: ticketList[i].voucher_type,
                ticketInterval: [
                  ticketList[i].pzstart_date,
                  ticketList[i].pzend_date,
                ],
                ticketSum: ticketList[i].voucher_amt,
                ticketAccessoryList: [],
              };
              let fileList = ticketList[i].ticketAccessoryList;
              for (let index = 0; index < fileList.length; index++) {
                const element = fileList[index];
                for (let j = 0; j < element.fileName.split("\\b").length; j++) {
                  const map = {
                    name: element.fileName.split("\\b")[j],
                    bucket: element.bucket,
                    buctetKey: element.buctetKey.split("\\b")[j],
                    type: this.getFileType(element.fileName.split("\\b")[j]),
                    status: "success",
                  };
              ...
              // for (let j = 0; j < ticketList[i].ticketAccessoryList[i].fileName.split("\\b").length; j++) {
              //   const map = {
              //     name: ticketAccessoryList[i].fileName.split("\\b")[j],
              //     bucket: ticketAccessoryList[i].bucket,
              //     buctetKey: ticketAccessoryList[i].buctetKey.split("\\b")[j],
              //     type: this.getFileType(ticketAccessoryList[i].fileName.split("\\b")[j]),
              //     status: "success",
              //   }
              //   ele.ticketAccessoryList.push(map);
              // }
    ...
    ticketValidHandle() {
      ...
      this.applyForm.ticketList.forEach((item) => {
        let ticketRule = {
          ticketType: [{ required: true, message: "请选择凭证类型" }],
          ticketInterval: [{ required: true, message: "请选择凭证时间区间" }],
          ticketSum: [{ required: true, message: "请输入总金额" }],
          ticketAccessory: [
    ...
    accessoryChange(file, fileList, index) {
      ...
      if (!isLt8M) {
        this.applyForm.ticketList[index].ticketAccessoryList.splice(
      ...
      formData.append("org_id", userInfo.org_id);
        ...
        this.$http
          .invokeAPI("/SUMP/icmcall/file", "post", formData)
          .then((res) => {
            if (res && res.code === "200" && res.data && res.data.path) {
              ...
              file.type = file.name.substring(
                file.name.lastIndexOf(".") + 1,
                file.name.length
              );
              this.applyForm.ticketList[index].ticketAccessory.push({
                path: res.data.path,
                file_name: file.name,
              });
      ...
      if (file.bucket) {
        switch (file.type) {
          ...
          case "pdf":
            ...
            break;
          default:
            this.getBase64Url(file.buctetKey, file.bucket, file.type);
            break;
        }
      } else if (file.url) {
        switch (file.type) {
     ...
     */
    htfjUploadChange(file, fileList) {
      console.log(fileList, "ppppppppp");
      this.htfjFileList = fileList; // 记录当前文件列表
      const isLt8M = file.size / 1024 / 1024 < 8;
      if (!isLt8M) {
        this.htfjFileList.splice(fileList.indexOf(file), 1); // 文件大小校验不通过 移除记录的文件列表中的文件
        this.$message({
          type: "warning",
          message: "证明材料文件大于8M",
        });
        ...
        this.$http
          ...
          .then((res) => {
            if (res && res.code === "200" && res.data && res.data.path) {
              ...
              this.applyForm.htfjUploadList.push({
                path: res.data.path,
                file_name: file.name,
              }); // 记录上传成功文件的文件路径
              if (fileList.length === 1) {
                this.$refs.applyForm.validateField("htfjUploadList"); // 手动触发表单校验
              }
            } else {
              this.htfjFileList.splice(fileList.indexOf(file), 1); // 文件上传失败 移除记录的文件列表中的文件
              this.$message({
                type: "error",
                message: "上传失败！",
              });
     ...
     * @param fileList 已添加文件集合
     */
    htfjUploadRemove(file, fileList) {
      let index = null;
      this.htfjFileList.forEach((item, key) => {
        // 找到移除文件对象下标
        if (item.uid === file.uid) {
          index = key;
          return false;
      ...
      this.applyForm.htfjUploadList.splice(index, 1); // 移除记录的文件路径列表中的文件
      this.htfjFileList.splice(index, 1); // 移除记录的文件列表中的文件
      if (fileList.length === 0) {
        this.$refs.applyForm.validateField("htfjUploadList"); // 手动触发表单校验
      }
    },
     ...
     * @param callback 回调方法
     */
    htfjUploadValidate(value, rule, callback) {
      // if (this.voucherForm.htfjUploadList.length === 0) {
      //   return callback(new Error("请上传签订合同附件文件"));
      // }
      return callback();
    },
        ...
        this.$http
          ...
          .then((res) => {
            if (res && res.code === "200" && res.data && res.data.path) {
              ...
              this.applyForm.credentialList.push({
                path: res.data.path,
                file_name: file.name,
              }); // 记录上传成功文件的文件路径
              if (fileList.length === 1) {
    ...
    credentialRemove(file, fileList) {
      let index = null;
      this.credentialFileList.forEach((item, key) => {
        // 找到移除文件对象下标
        if (item.uid === file.uid) {
          index = key;
          return false;
        ...
        this.$http
          ...
          .then((res) => {
            if (res && res.code === "200" && res.data && res.data.path) {
              ...
              this.voucherForm.aaumUploadList.push({
                path: res.data.path,
                file_name: file.name,
              });
              if (fileList.length === 1) {
