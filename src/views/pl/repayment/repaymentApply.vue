<template>
  <div>
      ...
      <el-collapse-item name="2">
        ...
        >
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item :label="$i18ns('应计罚息')">
                <el-input v-model="debtInfoForm.ysfx" :disabled="true"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item :label="$i18ns('应收费用')">
                <el-input v-model="debtInfoForm.recv_fee" :disabled="true"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item :label="$i18ns('提前还款锁定期(天)')">
                <el-input v-model="debtInfoForm.adv_rpymt_lock_prd" :disabled="true"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item :label="$i18ns('允许提前还款')">
            ...
            <el-col :span="6">
              <el-form-item :label="$i18ns('还款金额')" prop="repay_amtl">
                <el-input v-model="debtInfoForm.repay_amtl"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item :label="$i18ns('罚金金额')" prop="adv_rpymt_fine_amt" class="item-flex-box">
                <el-input v-model="debtInfoForm.adv_rpymt_fine_amt" :disabled="true"></el-input>
                <el-button :disabled="(this.typeForm.rpymt_catg === '3' && this.debtInfoForm.repay_amtl) ? false : true" @click="InitialWhenBlur2">试算</el-button>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
              ...
              <el-col :span="6">
                <el-form-item prop="management_approval_has_been_obtained" class="checkbx-label-style">
                  <el-checkbox-group v-model="repayForm.management_approval_has_been_obtained">
                    <el-checkbox label="已获得管理层审批" name="management_approval_has_been_obtained"
                                 :disabled="ifwrite"></el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="还款方式" prop="rpymt_catg">
                <el-select v-model="typeForm.rpymt_catg" placeholder @change="catgSelect">
                    <el-option
            ...
            <el-col :span="8">
              <el-form-item label="提前还款方式" prop="adv_rpymt_catg">
                <el-select v-model="typeForm.adv_rpymt_catg" placeholder :disabled="catgDisabled">
                    <el-option
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="提前还款调整计划方式" prop="adv_rpymt_adjust_plan_way">
                <el-select v-model="typeForm.adv_rpymt_adjust_plan_way" placeholder :disabled="catgDisabled">
                    <el-option
            ...
            <el-col :span="8">
              <el-form-item label="提前还款还息方式" prop="adv_rpymt_int_rpymt_way">
                <el-select v-model="typeForm.adv_rpymt_int_rpymt_way" placeholder  :disabled="catgDisabled">
                    <el-option
    ...
    <div class="custom-common--footer">
      ...
      <el-button type="primary" size="small" @click="checkAll('submit')" :disabled="sfPressbut">{{ $i18ns('提交') }}
      </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      debtInfoForm: {
        ...
        recv_fee: "", //逾期违约金
        is_allow_adv_settle: "", //允许提前还款
        repay_acct: "",//还款账号
        // is_allow_part_repay: "", //允许提前部分还款
        dflt_rpymt_acct_no: "",//银行卡号
        repay_amtl: "",
        adv_rpymt_lock_prd: "",//提前还款锁定期（天）
        strt_int_caln_dt: "",//起息日期
        slbl_prod_code: "",//可售产品代码
        rvlg_loan_flg: "",//循环贷款标志
        use_lmt_flg: "",//使用额度标志
      ...
      catgDisabled: false // 还款种类 还款方式联动
    };
  },
  activated() {
    // 路由传参进入页面刷新页面
  ...
  methods: {
    ...
    /**
     * 还款种类 还款方式为提前还款
     * 提前还款方式、提前还款调整计划方式、提前还款还息方式置灰且清空
     */
    catgSelect(val) {
      console.log("========", val)
      if(val !== '3') {
        this.catgDisabled = true
        this.typeForm.adv_rpymt_catg = ""
        this.typeForm.adv_rpymt_adjust_plan_way = ""
        this.typeForm.adv_rpymt_int_rpymt_way = ""
      }else {
        this.catgDisabled = false
      }
    },
     ...
     */
    InitialWhenBlur2() {
      if (!this.debtInfoForm.repay_amtl || this.isNull(this.debtInfoForm.repay_amtl)) {
        return false;
      }
      this.calculateFine();
    },
    ...
    getInitialInfo() {
      let params = {
        ...
        adv_rpymt_fine_amt: this.debtInfoForm.adv_rpymt_fine_amt
      }
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(
        res => {
          if (res && res.code === "200" && res.data) {
            ...
            this.debtInfoForm.adv_rpymt_lock_prd = data.adv_rpymt_lock_prd; //提前还款锁定期（天）
            this.debtInfoForm.strt_int_caln_dt = data.strt_int_caln_dt //起息日期
            this.debtInfoForm.slbl_prod_code = data.slbl_prod_code //可售产品代码
            this.debtInfoForm.rvlg_loan_flg = data.rvlg_loan_flg //循环贷款标志
            this.debtInfoForm.use_lmt_flg = data.use_lmt_flg//使用额度标志
            ...
            this.debtInfoForm.repay_acct = data.rpymt_acct_num; //还款账号
            ...
            this.debtInfoForm.ysfx = data.chk_write_off_prcpl;//罚息

            this.debtInfoForm.collt_debit_inst = data.urge_collt_db_int;
            this.debtInfoForm.is_allow_adv_settle = data.alw_adv_rpymt; //是否允许提前还款
    ...
    calculateFine() {
      ...
      let params = {
        servicecode: "ln3197",
        loan_due_bill_num: this.basicForm.loan_no,
        rpymt_catg: this.typeForm.rpymt_catg,
        adv_rpymt_catg: this.typeForm.adv_rpymt_catg,
        rpymt_amt: this.debtInfoForm.repay_amtl
      }
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200" && res.data) {
            this.debtInfoForm.adv_rpymt_fine_amt = res.data.adv_rpymt_fine_amt;
          }
        })
        .catch(err => {
          console.error(err);
