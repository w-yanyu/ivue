  ...
  <div>
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="3">
          ...
          <div style="padding:16px;">
            <el-button type="primary"  @click="addPayApply()">{{$i18ns('新增')}}</el-button>
          </div>
          <el-table ref="modifyTable" :data="params.list01" :border="true"  @selection-change="handleSelection" stripe>
            ...
            <el-table-column :label="$i18ns('操作')">
              <template slot-scope="scope">
                <!-- 点击编辑进入编辑页面进行编辑表格数据 -->
                <el-button size="small" type="danger" @click="handleDelete(scope.$index, scope.row)">{{$i18ns('删除')}}</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div class="space"></div>
    ...
    <div class="custom-common--footer">
      <el-row class="custom-common--footer">
        ...
        >{{$i18ns('启动流程')}}</el-button>
        <el-button type="primary" size="small" @click="closeForm">{{$i18ns('取消')}}</el-button>
      </el-row>
    </div>
    <el-dialog :title="$i18ns('责任认定信息输入')"
               :visible.sync="dutyapplyVisible" width="70%"
               v-if="dutyapplyVisible" :close-on-click-modal="false"
               @close ="closeDia" :destroy-on-close="true"
               :append-to-body="true">
      <div class="container-content">
        <div class="container-content--search">
          ...
          >
            <el-row>
              <el-col :span="12">
                <el-form-item :label="$i18ns('责任人')" prop="duty_per_nm"  label-width="200px">
                  <el-input v-model="dutyForm.duty_per_nm" ></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              <el-form-item :label="$i18ns('责任种类')" prop="duty_tp"  label-width="200px">
              ...
              <el-col :span="12">
                <el-form-item :label="$i18ns('责任程度')" prop="duty_degree"  label-width="200px">
              ...
              <el-col :span="12">
                <el-form-item :label="$i18ns('责任人处理方式')" prop="deal_way"  label-width="200px">
              ...
              <el-col :span="24">
              <el-form-item :label="$i18ns('责任原因')" prop="duty_tp"  label-width="200px">
                <el-input
                  type="textarea"
                  resize="none"
                  :rows="3"
          ...
          <div style="padding:16px; text-align: center;">
            <el-button type="primary"  @click="submitDateMes">{{$i18ns('提交')}}</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      dutydeLst:[],
      dealwayLst:[],
      startFlag:true,//判断是否是流程启动
      rules: {
        duty_per_nm: [
          {required: true, message: "请输入责任人"}
        ],
        duty_tp: [
          {required: true, message: "请选择责任种类"}
        ],
        duty_degree: [
          {required: true, message: "请选择责任程度"}
        ],
        deal_way: [
          {required: true, message: "请选择责任人处理方式"}
        ],
        duty_cause: [
          {required: true, message: "请输入责任原因"}
        ],
      }
    }
  },
  ...
  methods: {
    ...
    submitForm() {
           this.params.remark=this.fileForm.remark
           this.params.cust_name=this.basicForm.cust_name
    ...
    lncfnoBlur() {
      ...
      this.params.loan_no=this.basicForm.loan_no;
      this.getLoanInfo();
    },
    /**
     * 文件上传
