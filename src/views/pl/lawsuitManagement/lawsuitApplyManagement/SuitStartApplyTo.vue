    ...
    <el-collapse v-model="activeNames" class="custom-common--collapse">
      <el-collapse-item name="1">
        ...
        >
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item :label="$i18ns('身份证号')" prop="idtfno">
            ...
            <el-col :span="6">
              <el-form-item :label="$i18ns('人行征信')">
                <el-button type="primary" size="small" @click="showReport">{{$i18ns('人行征信')}}</el-button>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <el-collapse-item name="3">
        ...
        >
          <el-row>
            <el-col :span="6">
              <el-form-item :label="$i18ns('诉讼主因')" prop="suitReason">
                <el-select v-model="applyForm.suitReason" placeholder>
                  <el-option
...
export default {
  ...
  methods: {
    ...
    bringBack(formItem, returnData) {
      ...
      //字典渲染
      this.getInitialInfo();
      console.log("loan_no ",data["loan_no"] , data["cust_no"]);
      if(data["cust_no"]){
        this.getUserInfo(data["cust_no"]);
      }
    },
    /**
     * 人行征信查询
    ...
    getUserInfo(cust_no) {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(
        res => {
          if (res && res.code === "200" && res.data) {
            let data = res.data;
            this.basicForm.idtfno = data.info.usbCustInfoCom.cert_no;
