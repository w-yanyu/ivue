<template>
  <div>
    <el-collapse v-model="activeNames" class="custom-common--collapse">
      <el-collapse-item name="1">
        ...
        >
          ...
          <div class="border-Line" v-show="false">
            <div class="table-title">{{$i18ns('配偶信息')}}</div>
            <el-row class="caSeq-table">
              <el-table :data="tableDataOne">
                <el-table-column :label="$i18ns('姓名')" align="center" prop="acctna"></el-table-column>
                <el-table-column :label="$i18ns('借据号')" align="center" prop="lncfno"></el-table-column>
                <el-table-column
                  :label="$i18ns('贷款状态')"
                  align="center"
                  prop="clssst"
                ...
                <el-table-column
                  ...
                  :formatter="transferData"
                ></el-table-column>
                <el-table-column :label="$i18ns('当前委案机构')" align="center" prop="nowvorgnm"></el-table-column>
                <el-table-column
                  :label="$i18ns('当前案件委案状态')"
                  align="center"
                  prop="entrust_status"
                  :formatter="transferData"
                ></el-table-column>
                <el-table-column
                :label="$i18ns('当前案件诉讼状态')"
                  align="center"
                  prop="suit_status"
                  :formatter="transferData"
                ></el-table-column>
      ...
      <el-collapse-item name="2">
        <template slot="title">
          ...
          <div class="warn-class">{{$i18ns('提示：诉讼委案后，贷款将强制到期，且不可逆!')}}</div>
        </template>
        <el-form
          ref="applyForm"
          :rules="rules"
        ...
        >
          <el-row>
            <el-col :span="6">
              <el-form-item :label="$i18ns('诉讼地')" prop="forum_willmigerl">
                <el-select v-model="applyForm.forum_willmigerl" placeholder>
                  <el-option
          ...
          <el-row>
            <el-col :span="6">
              <el-form-item :label="$i18ns('机构类型')" prop="entrorg_type">
                <el-select v-model="applyForm.entrorg_type" placeholder>
                  <el-option
                    v-for="(item, i) in entrorgtypeList"
                    :key="i"
                    :label="item.dictName"
                    :value="item.dictId"
                    @click.native="getEntrorgList(areaCenter, entrorgtype)"
                  ></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item :label="$i18ns('委案机构')" prop="entrorg">
                <el-select @click.native="selEntrorg" v-model="applyForm.entrorg" placeholder>
                  <el-option
                    v-for="(item, i) in entrorgList"
                    :key="i"
                    :label="item.branch_name"
                    :value="item.branch_id"
                    @click.native="getEntrorgName(item)"
                  ></el-option>
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="6">
              <el-form-item :label="$i18ns('费用结算模式')" prop="clen_mode">
                <el-select v-model="applyForm.clen_mode" placeholder clearable>
                  <el-option
    ...
    <div class="custom-common--footer">
      <!-- <el-button type="primary" size="small" @click="checkAll('save')" :disabled="sfPressbut">保存</el-button> -->
      ...
      <!-- <el-button size="small" @click="cancelApply">取消</el-button> -->
    </div>
    <!-- 人行征信 -->
    <credit-report-info
...
export default {
  ...
  methods: {
    ...
    getEntrorgName(item){
      this.applyForm.entrorg_name=item.branch_name
    },
    ...
    getEntrorgList(areaCenter, entrorgtype) {
      var params = {
        ...
        mech_status: "YDR" ,//存续状态
        orgType:this.applyForm.entrorg_type //机构类型 
      };
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200") {
