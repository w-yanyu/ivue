    ...
    <el-collapse v-model="activeNames" class="custom-common--collapse">
      ...
      <el-collapse-item name="2">
        ...
        >
          <el-row>
            <el-col :span="6">
              <el-form-item label="关闭申请原因" prop="close_reason">
                <el-select v-model="applyForm.close_reason" placeholder>
                  <el-option
                    v-for="(item, i) in closeReasonList"
                    :key="i"
                    :label="item.dictName"
                    :value="item.dictId"
                    :disabled="true"
...
export default {
  ...
  mounted() {
    ...
    this.getDictList("PL_E_CLOSEREASON", "closeReasonList"); //关闭原因字典
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(
        res => {
          if (res && res.code === "200" && res.data) {
            ...
            this.applyForm.closeReason = data.closeReason
              ? data.closeReason
              : "";
