    ...
    <el-collapse v-model="activeNames" class="custom-common--collapse">
      ...
      <el-collapse-item name="3">
        <template slot="title">
          ...
          <div class="warn-class">{{$i18ns('提示：该申请单提交结案后，将无法进行任何借款缴费!')}}</div>
        </template>
        <el-form
          ref="applyForm"
          :rules="rules"
            ...
            <el-row class="caSeq-table">
              <el-table :data="tableData">
                <el-table-column
                  ...
                  align="center"
                  prop="fee_type"
                  :formatter="feetypeTranslate"
                ></el-table-column>
                <el-table-column :label="$i18ns('费用金额')" align="center" prop="pay_amount"></el-table-column>
                <el-table-column :label="$i18ns('收款账户名称')" align="center" prop="rece_accno"></el-table-column>
                <el-table-column
                  :label="$i18ns('缴费确认状态')"
                  align="center"
                  prop="pay_confirm_status" 
                  :formatter="confirmstatusTranslate"
                ></el-table-column>
                <el-table-column :label="$i18ns('客户承担金额')" align="center" prop="cust_amt"></el-table-column>
                <el-table-column :label="$i18ns('法院退回金额')" align="center" prop="court_back_amt"></el-table-column>
                <el-table-column :label="$i18ns('已处理法院退回金额')" align="center" prop="court_back_balance_after"></el-table-column>
                <el-table-column :label="$i18ns('未处理法院退回金额')" align="center" prop="court_back_balance"></el-table-column>
                <el-table-column :label="$i18ns('公司承担金额')" align="center" prop="compnay_amt"></el-table-column>
                <el-table-column :label="$i18ns('已处理公司承担金额')" align="center" prop="compnay_balance_after"></el-table-column>
                <el-table-column :label="$i18ns('未处理公司承担金额')" align="center" prop="compnay_balance"></el-table-column>
              </el-table>
            </el-row>
          </div>
          <el-row>
            <el-col :span="10" class="checkbx-label-style">
              <el-form-item prop="ispayover" :label="$i18ns('该客户需偿还金额均已清偿完毕，现客户可结清')">
                <el-checkbox-group v-model="applyForm.ispayover">
                  <el-checkbox :label="$i18ns('确认')" name="ispayover"></el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </el-col>
          </el-row>
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        ispayover: [
          {
            required: true,
            message: "请勾选已确认该客户需偿还金额均已清偿完毕，现客户可结清"
          }
        ]
      },
      ...
      /* ---------历史缴费记录------------------------*/
      tableData: [], // 历史缴费记录列表
      tableTitle: this.$i18ns('客户历史缴费记录'), //表格标题
  ...
  mounted() {
    ...
    this.getDictList("E_FEE_TYPE", "feetypeList"); //费用类型字典
    this.getDictList("E_PAY_CONFIRM_STATUS", "confirmstatusList"); //确认缴费状态字典
  },
  ...
  methods: {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(
        res => {
          if (res && res.code === "200" && res.data) {
            ...
            this.tableData = data.payfeehist;
            this.applyForm.ispayover = data.ispayover ? data.ispayover : "";
    ...
    transformData() {
      ...
      params.ispayover = params.ispayover ? "1" : "0";
      return params;
    },
    /**
     *提交表单数据
    ...
    checkAll(val) {
      ...
      this.$refs.basicForm.validate(valid => {
        ...
        //2.校验表单必须输入项是否填写
        this.applyForm.ispayover = this.applyForm.ispayover
          ? this.applyForm.ispayover
          : "";
     ...
     * 确认缴费类型转换
     */
    confirmstatusTranslate(row, column) {
      let name = row[column.property];
      let data = row[column.property];
      this.confirmstatusList.forEach(function(item, index) {
        if (data == item.dictId) {
          name = item.dictName;
        }
      });
