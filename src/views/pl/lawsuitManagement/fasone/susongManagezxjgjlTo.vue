        ...
        <el-collapse-item name="3">
          ...
          >
           ...
           <!--  <el-col :span="12">
                ...
                >
                  <el-input v-model="appForm.org_no"   @change="implementNoChange"  :disabled="true"></el-input>
                </el-form-item>
              </el-col>-->
              <el-col :span="12">
                <el-form-item
                  ...
                  prop="org_no"
                >
                  <el-select v-model="appForm.org_no" placeholder=" " :disabled="true" >
                    <el-option
              ...
              <el-col :span="12">
                ...
                >
                  <el-input v-model="appForm.ma_org_no" @change="implementNoChange" :disabled="true" ></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="12">
                ...
                >
                  <el-select
                    v-model="appForm.exe_file_type"
                    placeholder="请选择执行文件类型" :disabled="true"
                  >
                    <el-option
                      v-for="(item, i) in zxfiletypeList"
                      :key="i"
                      :label="item.dictName"
                      :value="item.dictId"
                       :disabled="true"
              ...
              <el-col :span="12">
                ...
                >
                  <el-input v-model="appForm.exe_result_no"   @change="implementNoChange"  :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item
            ...
            <el-row>
              ...
              <el-col :span="12" v-show="false">
                ...
                >
                  <upload-file
                    ref="zxwjinfoProp"
                    :uploadFileList="appForm.zxwjinfo"
                    :uploadDisabled="false"
                    @changeFile="zxwjinfoChange"
                  ></upload-file>
                </el-form-item>
              </el-col>
              <!-- <el-col :span="12">
                <el-form-item label="执行文件" prop="zxwjinfo" :rules="[{ required: true, message: '请上传执行文件' }]">
                  <personal-return-upload
                    ref="zxwjinfoProp"
                    :uploadFileList="appForm.zxwjinfo"
                    :uploadDisabled="false"
                    @changeFile="zxwjinfoChange"
...
//import changeHisListzxjglu from "@/views/fs/susongManage/changeHisListzxjglu.vue";
//import susongManagezxsljlViewTest from '@/views/fs/susongManage/susongManagezxsljlViewTest.vue';

export default {
  name: "susongManagezxjgjl",
  components: {
   // UploadFile,
    //PersonalReturnUpload,
   // changeHisListzxjglu,
  ...
  data() {
    return {
      ...
      zxfiletypeList: [], //执行文件类型下拉
      isunitlnList: [], //是否联合贷列表
      entrust_statusList: [], //当前案件委案状态列表
      suit_statusList: [], //当前案件诉讼状态列表
      under_agencyList: [], //承办机构名称列表
      shoul_agencyList: [], //案件受理机构名称列表
  ...
  created() {
    ...
    this.getDictList("PL_E_ZXFILETP", "zxfiletypeList"); // 执行文件类型
...
          tem = "";
        }
         this.appForm.zxjgid = tem;
         return this.appForm.zxjgid;
     },
      ...
      .then((res) => {
        if (res && res.code === "200") {
         this.appForm.org_no = res.data.org_no;
          ...
          this.appForm.exe_file_type = res.data.exe_file_type;
          this.appForm.exe_result_no = res.data.exe_result_no;
          this.appForm.exe_file_tm = res.data.exe_file_tm;
    ...
    // initRule(){
    //   this.appRules = {
    //       zxwjinfo:[{ validator: this.zxwjinfoValidate, trigger: 'change' }],
    //   }
    // },
    /**
     * 获取数据字典列表
    ...
    getBasciInfo() {
      ...
      //   this.appForm.zxwjinfo = [];
      //   this.$refs.zxwjinfoProp.clearFileList();
      // }
      let params = {
        servicecode: "ln3100",
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then((res) => {
          if (res && res.code === "200" && res.data) {
            ...
            // this.basicForm.clssst = response.clssst; //贷款形态
            // this.basicForm.areaCenter = response.areaCenter; // 区域中心
            // this.basicForm.ovdays = response.ovdays; // 逾期天数
    ...
    //利用借据号查询执行受理记录表中的最新的执行受理流水号
    getzxslnumber() {
      let params = {
        servicecode: "RLMSPLZH0016",
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params).then(
        (res) => {
          if (res && res.code === "200" && res.data) {
            const response = res.data;
            this.appForm.zxslnumber = response.zxslnumber;
          }
        }
      );
    },
    ...
    //执行结果记录申请信息提交
    submit() {
      // if(this.appForm.zxwjinfo ==""){
      //       this.$message({type: "error", message: "执行文件为空，请上传附件文件"})
      //       return false
      // }
      // if(this.appForm.under_agency ==""){
      //       this.$message({type: "error", message: "承办机构为空，请选择承办机构"})
      //       return false
      // }
        ...
        if (valid) {
          this.$confirm("是否确认提交申请?", "提示", {
            ...
            let params = {
              ...
              zxslnumber: this.appForm.zxslnumber,
              under_agency: this.appForm.under_agency, //承办机构
              shoul_agency: this.appForm.shoul_agency, //受理机构
              zxfiletype: this.appForm.zxfiletype, //执行文件类型
              zxjgid: this.appForm.zxjgid, //执行结果案号
              zxwjtime: this.appForm.zxwjtime, //执行文件落款时间
              zxwjinfo: this.appForm.zxwjinfo, //执行附件
