<template>
  <div class="page_content">
    <div class="horizontal_title">
      <div> 通用类资产负债表(新会计) </div>
      <div> 日期 2007年02月28日 </div>
      <div> 单位：元 </div>
    </div>
    ...
    <table class="tabel_balance">
      ...
      <tr class="tr_content" v-for="(itemData,index) in tableData" :class="{'link-item-active': itemData.id == activeLinkId,'link-item-hover':index==hoverIndex}"
        ...
        <td class="textAginleft">{{itemData.rName}}</td>
        <td>{{itemData.rIndex}}</td>
        <td>
          <el-input class="table_content_input" v-model="itemData.rLastTerm" :disabled="itemData.rLastTermDisable=='0'">
          </el-input>
        </td>
        <td>
          <el-input class="table_content_input" v-model="itemData.rThisTerm" :disabled="itemData.rThisTermDisable=='0'">
          </el-input>
        </td>
      </tr>
