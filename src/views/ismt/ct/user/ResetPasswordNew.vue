<template>
  <div>
    <el-form :model="form" :rules="rules" ref="ruleForm" class="update-password">
      <el-form-item label="用户id" label-width="240px" prop="user_id">
        <el-input
      ...
      <el-form-item class="update-password--footer">
        <el-button class="update-password--confirm" type="primary" plain @click="modify('ruleForm')">{{translate('GuKk')}}</el-button>
...
export default {
  ...
  data() {
    let checkNew = (rule, value, callback) => {
      ...
      if (this.form.confirm_user_password !== "" && value !== this.form.confirm_user_password) {
  ...
  created() {
    this.getPublicKey();
    this.submitForm.user_id = this.cParentParams.user_id;
    this.form.user_id = this.cParentParams.user_id;
  },
  methods: {
    /**
     * 获取rsa加密的公钥
    ...
    modify(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.submitForm.user_id = this.form.user_id;
          this.submitForm.confirm_user_password = sha1(this.form.confirm_user_password);
          this.$http.invokeAPI("/SUMP/rest/auth/rstpwd", "post", this.submitForm)
          .then(response => {
            if (response && response.code === "200") {
