<template>
  <div>
    <div>     
      <to-json
            toUrl="/views/pd/prodTemplate/prod_template_info.json"
            :params="basicForm">
          </to-json>
    </div>
    <div class="todo-top">
...
export default {
  ...
  data() {
    return {
      产品模板编号:this.$i18ns("产品模板编号"),
      产品模板类型:this.$i18ns("产品模板类型"),
      产品模板名称:this.$i18ns("产品模板名称"),
      版本号:this.$i18ns("版本号"),
      是否有效:this.$i18ns("是否有效"),
      适用客户类型:this.$i18ns("适用客户类型"),
  ...
  created() {
    ...
    this.basicForm.cust_class = this.cParentParams.cust_class;
  },
  mounted() {
    //初始化数据字典
    //this.getDictList("MS_E_YESORNO", "isYESORNOList"); //获取数据字典MS_E_YESORNO
    //this.getDictList("PD_E_CUST_CLASS", "custClassList");
  },
  methods: {
    /**
     * 获取数据字典列表
    ...
    //  async getTableData(apply_type_list) {
    //     apply_type_list.forEach(async (apply_type) => {
    //       let params = {
    //         servicecode: "pd0143",
    //         grupno: "0006",
    //         id: apply_type,
