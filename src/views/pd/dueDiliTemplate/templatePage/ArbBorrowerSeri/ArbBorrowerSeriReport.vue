  ...
  <div class="table_container">
    <div class="table_center">
      <table border="1" class="table_report">
        <tr>
          <th colspan="14" class="table_header">借款人投资服务业情况</th>
        </tr>
        <tr>
          <!--          <th>房产<br>类别</th>-->
          <th>投资企业<br>名称</th>
      ...
      <table border="1" class="table_report">
        <tr>
          ...
          <th>经营<br>规模</th>
          <th>知名度</th>
          <th>当地市<br>场行业<br>地位</th>
          <th>行业管理部门<br>规定的各项经<br>营许可证是否<br>已办妥</th>
          <th>经营手<br>续是否<br>齐全</th>
        </tr>
        <tr v-for="(item,index) in tableDataList"
            :key="index">
          <td>{{ item.busi_item }}</td>
          <td>{{ getDictLabel('yesOrNOList', item.is_inve_fixa) }}</td>
          <td>{{ item.oper_scale }}</td>
          <td>{{ item.popularity }}</td>
          <td>{{ item.locm_indp }}</td>
          <td>{{ getDictLabel('yesOrNOList', item.is_oper_credit) }}</td>
          <td>{{ getDictLabel('yesOrNOList', item.is_oper_proc) }}</td>
        </tr>
      </table>
    </div>
  </div>
