  ...
  <div class="table_container">
    <div class="table_center">
      <table border="1" class="table_report">
        <tr>
          <th colspan="14" class="table_header">借款人投资商贸企业情况</th>
        </tr>
        <tr>
          <th>投资企业<br>名称</th>
          <th>注册<br>时间</th>
        ...
        <tr v-for="(item,index) in tableDataList"
          ...
          <td>{{ item.oper_way }}</td>
          <td>{{ item.oper_main_prod }}</td>
          <td>{{ getDictLabel('yesOrNOList', item.is_rega_oper) }}</td>
        </tr>
      </table>
      <el-row>
        &nbsp;
      ...
      <table border="1" class="table_report">
        ...
        <tr v-for="(item,index) in tableDataList"
          ...
          <td>{{ item.upst_partner }}</td>
          <td>{{ item.down_partner }}</td>
          <td>{{ item.updo_seme }}</td>
          <td>{{ item.busi_manc }}</td>
          <td>{{ item.mana_level }}</td>
          <td>{{ item.busi_empl_number }}</td>
          <td>{{ item.oper_place }}</td>
          <td>{{ item.ware_condition }}</td>
        </tr>
      </table>
    </div>
  </div>
...
export default {
  name: "ArbBorrowerInveBuseReport",
  props: {
    cMeta: Object, // 布局相关数据
