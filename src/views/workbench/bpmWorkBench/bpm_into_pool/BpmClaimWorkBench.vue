        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button type="text" @click="claimCase(scope.row)">认领</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  methods: {
    ...
    claimCase(row) {
      if(row.currAprovRole.indexOf('DATA_VERY')!==-1){ //资料核实
        const params = {
          servicecode: "ICMSJKTS0170", // 认领接口
          usercd: JSON.parse(localStorage.getItem("user_info")).userCode,
          tkinid: row.taskInstId,
          applid: row.applid,
          isSpoloan: row.isSpoloan,
        };
        this.$http
          .invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
          .then((res) => {
