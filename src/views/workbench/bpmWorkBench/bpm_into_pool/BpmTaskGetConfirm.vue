...
export default {
  ...
  methods: {
    ...
    confirm() {
      const params = {
        servicecode: "clmtsk", // 认领接口
        usercd: JSON.parse(localStorage.getItem("user_info")).userCode,
        // applyId: this.cParentParams.applyId,
        tkinid: this.cParentParams.taskInstId,
      }
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200") {
            ...
            this.$publicFn.openNavTab("@????????????????????????????????", "待办事项办理", this.cParentParams, that); // 领取任务成功后跳转至待办事项办理页面
          }
        })
        .catch(err => {
          console.error(err);
