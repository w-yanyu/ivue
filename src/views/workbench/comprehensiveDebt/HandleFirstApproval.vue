  ...
  <div class="data-check--container">
      ...
      <el-collapse v-model="activeNames" class="data-check--collapse">
        <el-form ref="ruleForm" :model="ruleForm" label-width="160px" class="custom-common--form">
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="风险级别">
                  <el-input v-model="ruleForm.loanInfo.riskLevel" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item label="利率方案">
                  <el-input v-model="ruleForm.loanInfo.intrestType" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="最终额度">
                  <el-input v-model="ruleForm.sysAppResults.crdtAmt" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="利率">
                  <el-input v-model="ruleForm.sysAppResults.intRate" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="最高可贷金额">
                  <el-input v-model="ruleForm.sysAppResults.highestLoanQuota" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-collapse-item>
          <el-collapse-item name="15">
            ...
            <el-table
             :data="ruleForm.patchHstList"
            >
              <el-table-column prop="patchUserName" label="执行人"></el-table-column>
              <el-table-column prop="createTime" label="补件时间"></el-table-column>
              <el-table-column prop="patchNames" label="补件类型"></el-table-column>
          ...
          <el-collapse-item name="16" disabled>
            <template slot="title">
              <span>
                <em></em>人工审批意见
              </span>
                ...
                <el-row>
                  <el-col :span="12">
                    <el-form-item label="申请额度">
                      <el-input v-model="submitForm.applAmt" :disabled="true"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
                  ...
                  <el-col :span="24">
                    <el-form-item label=" ">
                      ...
                      <el-radio v-model="submitForm.commitType" @change="commitTypeChange" label="SUPPLY">待补件（补附件）</el-radio>
                      <el-radio v-model="submitForm.commitType" @change="commitTypeChange" label="REBACK">退回重审</el-radio>
                    </el-form-item>
                  </el-col>
                  <el-col :span="24" v-if="submitForm.commitType === 'REJECT'">
                    <el-form-item prop="rejectReasonList" label="拒绝原因">
                      <el-checkbox-group v-model="submitForm.rejectReasonList">
                        <el-checkbox v-for="(item, index) in rejectList" :key="index" :label="item.dictId">{{item.dictName}}</el-checkbox>
                      </el-checkbox-group>
                    </el-form-item>
                  </el-col>
                  ...
                  <el-col :span="24" v-if="submitForm.commitType === 'REBACK'">
                    <el-form-item prop="rebackReasons" label="退回原因">
                      <el-checkbox-group v-model="submitForm.rebackReasons">
                        <el-checkbox v-for="(item, index) in roleBackReviewList" :key="index" :label="item.dictId">{{item.dictName}}</el-checkbox>
                      </el-checkbox-group>
                    </el-form-item>
                  </el-col>
                  <el-col :span="24">
    ...
    <el-dialog
     ...
     v-if="relevantDialogShow">
      ...
      <education-detail :apply-id="cParentParams.applid" :apply-no="cParentParams.applyNo" v-if="relevantIndex === 1"/>
...
<script>
import ClaimdRecord from "./ClaimdRecord"; // 认领记录组件
import EducationDetail from "./EducationDetail"; // 学历学籍信息组件
import ApprHistory from "./ApprHistory";//审批历史
import SysJudgResult from "./SysJudgResult"; // 系统判断结果组件
import ReturnPatch from "./ReturnPatch"; // 退补件历史组件
import CertnoInfo from "./CertnoInfo"; //身份证信息
...
export default {
  ...
  data() {
    return {
      ...
      relevantList: ["认领记录", "学历学籍信息查询", "审批历程信息", "人工审批结果", "历史申请/重复性检查", "系统判断结果", "退补件历史", "身份证信息", "人脸比对结果"], // 相关资料列表
  ...
  computed: {
    ...
    roleBackReviewList() {
      let list = ["4"];
      return this.backReviewList.filter(item => {
        if (list.indexOf(item.dictId) > -1) {
          return true;
        }
        return false;
      })
  ...
  created() {
    ...
    this.getRepeatState(); // 获取重复性状态检查结果
    this.getReturnList(); // 获取补件信息
    ...
    this.getDictList("E_REPEAT", "repeatList"); // 初始化是否重复数据字典
    this.getDictList("E_REJECT", "rejectList"); // 初始化拒绝原因数据字典
  },
  methods: {
    /**
     * 获取资料核实信息
      ...
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200" && res.data) {
            ...
            this.ruleForm.sysAppResults = {
              crdtAmt: res.data.otherInfo.crdtAmt,
              intRate: res.data.intRate,
              highestLoanQuota: res.data.otherInfo.highestLoanQuota,
            }
          }
        })
        .catch(err => {
    ...
    getRepeatState() {
      ...
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200" && res.data && res.data.status === "S") {
    ...
    getReturnList() {
      ...
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200" && res.data) {
            this.patchPagination.tableTotal = res.data.patchHstList.length;
            const start = this.patchPagination.tableSize * (this.patchPagination.currentPage - 1);
            this.patchPagination.isShowPage = res.data.length < 10 ? false : true;
            this.ruleForm.patchHstList = res.data.patchHstList.splice(start, start + this.patchPagination.tableSize);
          }
        })
        .catch(err => {
          console.error(err);
    ...
    */
    ...
    /**
     * 合规处理点击事件
     * @param isCompliant 是否合规 Y：合规 N：不合规
     */
    handleCompliant(isCompliant) {
      for (let index = 0; index < this.videosList.length; index++) {
        const element = this.videosList[index];
        element.isCompliant = isCompliant;
      }
    },
        ...
        if (valid) {
          ...
          switch (params.commitType) {
            ...
            case "REJECT":
              ...
              for (let index = 0; index < this.rejectList.length; index++) {
                const element = this.rejectList[index];
                map[element.dictId] = element.dictName;
              }
              for (let index = 0; index < this.submitForm.rejectReasonList.length; index++) {
                const element = this.submitForm.rejectReasonList[index];
                if (map[element]) {
              ...
              break;
            case "REBACK":
              params.rebackReasons = this.submitForm.rebackReasons.join(",");
              break;
            default:
              break;
          }
