    ...
    <div>
        <el-form ref="form" :model="form" label-width="100px">
            <el-tabs type="border-card">
                ...
                <el-tab-pane>
                    <span slot="label">
                        <i class="el-icon-date"></i> 设置得分
                    </span>
                    <functionSetAll
                        :cParentParams="functionSetAllData"
                        noShowConfirmBtn="true"
                        :disabled="disabled"
                        ref="functionSetAll"
                    ></functionSetAll>
                </el-tab-pane>
...
import functionSetAll from "@/components/FunctionSet/functionSetAll"; //导入自定义函数情况输入布局
...
export default {
    ...
    created() {
        let that = this;
        console.log(
            "父类传过来的数据risk_decision_rule_add_dialog",
            that.cParentParams
        );
        if (that.cParentParams) {
            //如果规则配置存在
            if (that.cParentParams.executeExpressNature !== "") {
                let jsonRule = JSON.parse(
                    this.cParentParams.executeExpressNature
                );
                console.log("转译过的jsonRule数据", jsonRule);
                if (jsonRule) {
                    this.ruleForm = jsonRule;
                }
            } else {
                that.ruleForm.conditionValueShow =
                    that.cParentParams.scoreAttribute; //回显条件选择值
                that.ruleForm.conditionValue =
                    that.cParentParams.scoreAttribute; //条件选择值
                that.ruleForm.conditionDesc =
                    that.cParentParams.scoreAttributeName; //条件选择后的转译解释
                that.ruleForm.conditionDataType =
                    that.cParentParams.scoreAttributeDataType;
            }
            //如果得分公式配置存在
            if (that.cParentParams.scorePointsNature !== "") {
                that.functionSetAllData.row.valueNatureExpress =
                    that.cParentParams.scorePointsNature;
                // that.functionSetAllData.dataType = that.cParentParams.scoreAttributeDataType;
            } else {
                // that.functionSetAllData.dataType = that.cParentParams.scoreAttributeDataType;
                that.functionSetAllData.title = "得分";
            }
            that.functionSetAllData.dataType = "Decimal";
            if (that.cParentParams.openType === "view") {
                that.disabled = true;
                this.rules = {};
            }
        }
        console.log("disabled====" + that.disabled);
        that.operatorSymbolNormal = operatorNormal.data;
        //初始化联动下拉列表选项
        that.getConditionsList();
    },
    computed: {
        //根据条件筛选
        operatorSymbolNormalFilter() {
            ...
            operatorClone.forEach((item, index, arrs) => {
                item.children.forEach((childrenItem, index, arrs) => {
                    if (
                        (childrenItem.value === "include" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "contains" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "startWith" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "endWith" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "substr" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "indexOf" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "length" &&
                            this.ruleForm.conditionDataType !== "String") ||
                        (childrenItem.value === "countstr" &&
                            this.ruleForm.conditionDataType !== "String")
                    ) {
    ...
    methods: {
        ...
        changeOperatorSymbol(value) {
            ...
            if (this.ruleForm.conditionDataType !== "Boolean") {
                ...
                for (let i = 0; i < this.operatorSymbolNormal.length; i++) {
                    if (this.operatorSymbolNormal[i].value === key) {
        ...
        confirm() {
            ...
            if (
                this.$refs.functionSetAll &&
                !this.$refs.functionSetAll.isCompleted()
            ) {
                return;
            }
            this.translationConfirm();
            let functionSetAllData = {};
            if (that.$refs.functionSetAll) {
                functionSetAllData = that.$refs.functionSetAll.getConfirmData();
            }
            let obj = {
                conditionOperate: that.ruleForm.operatorSymbolValue, //操作符
                conditionValue: that.ruleForm.itemCode, //条件表达式

                index: that.cParentParams.index,
                openType: that.cParentParams.openType,
                executeExpress: `${that.ruleForm.funcitonCode} ${that.ruleForm.itemCode}`, //可执行表达式函数体+表达式
                executeExpressDesc: that.ruleForm.itemDesc, //可执行表达式中文描述
                executeExpressDisplay: that.ruleForm.itemCode, //可执行表达式展示
                executeExpressNature: JSON.stringify(this.ruleForm), //规则条件的json字段

                scorePointsType: functionSetAllData.valueType, //评分类型（常量，变量，函数）
                scorePointsExpress: functionSetAllData.valueExpression, //得分表达式
                scorePointsDisplay: functionSetAllData.scorePoints, //得分表达式展示
                scorePointsNature: functionSetAllData.valueNatureExpress, //得分的json字段
            };
