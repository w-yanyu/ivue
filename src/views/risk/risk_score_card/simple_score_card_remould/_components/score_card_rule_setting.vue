...
export default {
    ...
    created() {
        ...
        if (that.cParentParams) {
            //如果规则配置存在
            if (
                that.cParentParams.executeExpressNature &&
                that.cParentParams.executeExpressNature !== ""
            ) {
                ...
                that.ruleForm.conditionValueShow =
                    that.cParentParams.conditionValueShow; //回显条件选择值
                that.ruleForm.conditionValue =
                    that.cParentParams.conditionValue; //条件选择值
                that.ruleForm.conditionDesc = that.cParentParams.conditionDesc; //条件选择后的转译解释
                that.ruleForm.conditionDataType =
                    that.cParentParams.conditionDataType;
            }
    ...
    methods: {
        ...
        confirm() {
            ...
            let obj = {
                ...
                executeFunctionCode:
                    that.ruleForm.funcitonCode.length > 0
                        ? `${that.ruleForm.funcitonCode}`
                        : "", //可执行的方法
                executeExpress: `${that.ruleForm.itemCode}`, //可执行表达式函数体+表达式
