<template>
    <div class="page_content">
                        ...
                        <el-row span="24">
                            <el-col span="4" hidden>
                                <el-form-item label="评分卡ID " prop="scoreCardId" label-width="100px">
                                    <el-input v-model="ruleForm.scoreCardId" @blur="Input($event)" disabled></el-input>
                                </el-form-item>
                            </el-col>

                            <el-col span="8">
                                <el-form-item label="复杂评分卡代码 " prop="scoreCardCode" label-width="120px">
                                    <el-input v-model="ruleForm.scoreCardCode" disabled></el-input>
                                </el-form-item>
                            </el-col>

                            <el-col span="8">
                                <el-form-item label="复杂评分卡名称 " prop="scoreCardName" label-width="120px">
                                    <el-input v-model="ruleForm.scoreCardName" maxlength="125"
                            ...
                            <el-col>
                                <el-form-item label="备注 " prop="description" label-width="120px">
                                    <el-input class="input_el" v-model="ruleForm.description" @blur="Input($event)"
                                        type="textarea" maxlength="200" rows="5" show-word-limit></el-input>
                                </el-form-item>
                            </el-col>
                        </el-row>
                        <!--   <el-row span="24">
                ...
                <el-tab-pane>
                    ...
                    <el-form class="table_content">
                        <div class="table_btn_div">
                            <el-button type="primary" class="addBtn" size="small" @click="addTableRemould">+
                                添加条件</el-button>
                            ...
                            <el-form-item label="总分计算方式 " prop="totalExecType" label-width="100px">
                                <el-select v-model="ruleForm.totalExecType" placeholder="请选择规则类型" @change="changeTypeValue">
                                    <el-option v-for="item in totalExecTypeList" :key="item.dictId" :label="item.dictName"
                                        :value="item.dictId"></el-option>
                                </el-select>
                            </el-form-item>
                        </div>
                        <el-table :data="parnentTableData" style="width: 100%" :row-key="getRowKey" border
                            :span-method="objectSpanMethod" @row-contextmenu="rightClick" @row-click="clickTableRow"
                            v-loading="loading">
                                ...
                                <template slot-scope="scope" class="attributeTable">
                                    ...
                                    <div class="btnDiv">
                                        <span class="el-tag el-tag--primary el-tag--mini" style="cursor: pointer;"
                                            @click="handleEdit(scope.$index, scope.row)">维护</span>
                            ...
                            <el-table-column prop="itemScoreAttribute" label="分值" align="center">
                                <template slot-scope="scope">
                                    <span>{{ DataTypeorFormat(scope.row.itemScoreAttributeDataType) }} : {{
                                        scope.row.itemScoreAttribute }}</span>
                                </template>
                            </el-table-column>
        ...
        <div id="menu" class="menuDiv">
            <ul class="menuUl">
                <li v-for="(item, index) in menus" :key="index" @click.stop="infoClick(index)">{{ item }}</li>
            </ul>
        </div>
    </div>
</template>
...
<script>
import operatorNormal from "@/views/risk/dictdata/operatorSymbolNormal";
import totalExecTypeDictdata from "@/views/risk/dictdata/totalExecType.json";
...
export default {
     ...
     */
        ...
        init() {
            ...
            this.ruleForm.riskOperationType = this.cParams.riskOperationType;//操作类型
        },
        //评分卡ID输入
        Input(event) {
            this.showSaveBtn();
 ...
 this.$refs.ruleForm.validate(valid => {
        ...
        reset() {
            ...
            this.ruleForm.scoreCardStatus = "I";
            this.ruleForm.description = "";
        },

        //请求表格数据deci020111->deci020212
        ...
        rightClick(row, column, event) {
            ...
            let tempClickObj = this.getPoint(event.target);
            //获取我们自定义的右键菜单
            ...
            this.menuTempRow = row; //将右键选择的行存储，等菜单选择执行
        },
        // table的左键点击当前行事件
        clickTableRow(row, column, event) {
            console.log("clickTableRow-----", row, column, event);
            let menu = document.querySelector("#menu");
            menu.style.display = "none";
        },
        // 自定义菜单的点击事件
        infoClick(index) {
            console.log("infoClick----", index);
            let menu = document.querySelector("#menu");
            switch (index) {
                case 0: //编辑
                    this.handleEdit("", this.menuTempRow);
                    break;
                case 1: //查看
                    this.handleView("", this.menuTempRow);
                    break;
                case 2: //删除
                    this.handleDelete("", this.menuTempRow);
                    break;
            }
            menu.style.display = "none";
        },
