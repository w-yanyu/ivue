    ...
    <div class="page_content">
        <el-form class="table_content" ref="ruleForm" :model="ruleForm" :rules="rules">
            <el-row span="24">
                ...
                <el-col span="8">
                    <el-form-item label="简单评分卡代码 " prop="scoreCardCode" label-width="120px">
                        <el-input v-model="ruleForm.scoreCardCode" disabled></el-input>
                    </el-form-item>
                </el-col>

                <el-col span="8">
                    <el-form-item label="简单评分卡名称 " prop="scoreCardName" label-width="120px">
                        <el-input v-model="ruleForm.scoreCardName" maxlength="125" @blur="Input($event)"></el-input>
                    </el-form-item>
                </el-col>
                <el-col span="8">
            ...
            <el-row span="24">
                <el-col>
                    <el-form-item label="备注 " prop="description" label-width="120px">
                        <el-input class="input_el" v-model="ruleForm.description" @blur="Input($event)" type="textarea"
                            maxlength="200" show-word-limit :autosize="{ minRows: 2, maxRows: 4 }"></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
        ...
        <el-form class="table_content">
            ...
            <el-table :data="parnentTableData" style="width: 100%" :row-key="getRowKey" border>
                                ...
                                <template slot-scope="scope">
                                    <el-select size="mini" style="width: 120px; margin-right: 0px;"
                                        ...
                                        <el-option v-for="item in conditionType" :key="item.value" :label="item.label"
                                            :value="item.value"
                                            :disabled="switchScoreDisabled(item.value, scope.row.scoreAttributeDataType)"></el-option>
                                    </el-select>
                                ...
                                <template slot-scope="scope">
                                    <div v-if="scope.row.isSet || scope.row.isRulesSet">
                                        <span class="el-tag el-tag--info el-tag--mini" style="cursor: pointer;"
                                            @click="setFormula(scope.row, scope.$index, true)">{{ scope.row.scoreMoldText ?
                                                scope.row.scoreMoldText : '得分' }}</span>
                ...
                <el-table-column prop="scoreModelCode" label="属性数据类型">
                    <template slot-scope="scope">
                        <el-input size="mini" v-if="scope.row.isSet" v-model="scope.row.scoreAttributeDataType"></el-input>
                ...
                <el-table-column prop="id" label="数据操作" align="center">
                    <template slot-scope="scope">
                        <span class="el-tag el-tag--info el-tag--mini" style="cursor: pointer;"
                            @click="changeTable(scope.row, scope.$index, true)">{{ scope.row.isSet ? "保存" : "维护" }}</span>
                        <span v-if="!scope.row.isSet" class="el-tag el-tag--danger el-tag--mini" style="cursor: pointer;"
                            @click="handleDeleteAll(scope.$index, scope.row)">删除</span>
                        <span v-else class="el-tag el-tag--mini" style="cursor: pointer;"
                            @click="CancelTable(scope.row, scope.$index, false)">取消</span>
                    </template>
                </el-table-column>
            </el-table>
