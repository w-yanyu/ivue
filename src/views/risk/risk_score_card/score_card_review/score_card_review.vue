  ...
  methods: {
    ...
    requestTableData() {
      ...
      let params = {
        ...
        servicecode: "deci020116",
        isList: true,
      };
      that.$http
        .invokeAPI("/SUMP/icmcall/icmRPCCall", "POST", params)
        .then((obj) => {
          that.ruleForm.descriptionBefore = obj.data[0].descriptionBefore;
          //that.ruleForm.descriptionAfter = obj.data.descriptionAfter;
          that.ruleForm.scoreCardNameBefore = obj.data[0].scoreCardNameBefore;
          that.ruleForm.scoreCardNameAfter = obj.data[0].scoreCardNameAfter;
          that.ruleForm.totalExecTypeBefore = obj.data[0].totalExecTypeBefore;
          that.ruleForm.totalExecTypeAfter = obj.data[0].totalExecTypeAfter;
    ...
    objectSpanMethodBefore({ row, column, rowIndex, columnIndex }) {
      if (columnIndex === 0 || columnIndex === 1) {
         ...
         */
         send(){
             this.$refs.ruleForm.validate(valid => {
               console.log("意见"+this.ruleForm.reviewOpinion)
               if(this.ruleForm.reviewOpinion === false){
             this.$message({
              type: 'error',
              message:  "请输入审核意见!"
