...
export default {
    ...
    created() {
        this.cParams = this.cParentParams.cParentParams;
    ...
    mounted() {
        ...
        if (this.insertStatus) {
            let tempObj = JSON.stringify(this.cParams.nature_expression);
            let jsonParse = JSON.parse(tempObj);
            let jsonTwo = JSON.parse(jsonParse);
            this.ruleData = jsonTwo;
            this.crtZhRule = this.cParams.rule_content_desc;
            this.crtOriginBizRule = this.cParams.executable_expression;
        }
    },
    methods: {
        showAddModal() {
