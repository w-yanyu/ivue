    ...
    <div class="page_content">
      <el-form class="table_content" ref="ruleForm" label-width="120px" :model="ruleForm" :rules="rules">
        <el-row :span="24">
          ...
          <el-col :span="6">
            <el-form-item label="规则组代码" prop="groupCode">
              <el-input size="medium" disabled v-model="ruleForm.groupCode"/>
            </el-form-item>
          </el-col>
          <el-col :span="6">
  ...
  <!--            <el-input size="medium" disabled v-model="ruleForm.reviewUser"/>-->
    ...
    <el-row>
        ...
        <el-col
          :span="10"
          v-if="
            ruleForm.operationType === 'U' || ruleForm.operationType === 'S'
          "
        >
          <span>{{
            ruleForm.operationType === "A" || ruleForm.operationType === "C"
              ? "切换前:"
              : "旧版本:"
          }}</span>
            <el-card>
              <DecisionRuleGroupReviewCard
          ...
          <!-- 新版本表格 -->
        <el-col
          :span="ruleForm.operationType==='U'|| ruleForm.operationType==='S'?12:24"
        >
          <span>{{
            ruleForm.operationType === "B" ? "切换后:" : "新版本:"
          }}</span>
            <el-card>
              <DecisionRuleGroupReviewCard
...
import DecisionRuleGroupReviewCard from "@/views/risk/risk_decision_rule/group/decision_rule_group_review_card.vue";
import MyAxios from "pte-ui/utils/MyAxios"; // 模板导入

export default {
  name: "DecisionRuleGroupReview1",
  components: {
    DecisionRuleGroupReviewCard,
  },
  props: {
  ...
  methods: {
    async init() {
      ...
      that.ruleForm.groupId = this.cParentParams.groupId;
      that.ruleForm.groupCode = this.cParentParams.groupCode;
      that.ruleForm.groupName = this.cParentParams.groupName;
    ...
    initCardData() {
      //1.初始化切换前数据
      let beforeData = {};
      beforeData.groupId = this.cParentParams.groupId;
      beforeData.groupCode = "2";
      beforeData.groupName = this.cParentParams.groupNameBefore;
      beforeData.updateUser = this.cParentParams.updateUser;
      beforeData.updateTime = this.cParentParams.updateTime;
      beforeData.description = this.cParentParams.descriptionBefore;
      beforeData.version = this.cParentParams.versionBefore;
      if (this.isEmpty(this.cParentParams.resourceBefore)) {
        beforeData.tableParams = [];
      } else {
        beforeData.tableParams = JSON.parse(this.cParentParams.resourceBefore);
      }
      this.beforeTableData = JSON.parse(JSON.stringify(beforeData));

      //1.初始化切换后数据
      let afterData = {};
      afterData.groupId = this.cParentParams.groupId;
      afterData.groupCode = this.cParentParams.groupCode;
      afterData.groupName = this.cParentParams.groupNameAfter;
      afterData.updateUser = this.cParentParams.updateUser;
      afterData.updateTime = this.cParentParams.updateTime;
      afterData.description = this.cParentParams.descriptionAfter;
      afterData.version = this.cParentParams.versionAfter;
      if (this.isEmpty(this.cParentParams.resourceAfter)) {
        afterData.tableParams = [];
      } else {
        afterData.tableParams = JSON.parse(this.cParentParams.resourceAfter);
      }
      this.afterTableData = JSON.parse(JSON.stringify(afterData));
