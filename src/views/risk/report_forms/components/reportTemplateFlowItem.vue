...
export default {
    ...
    getBranchList() {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
         ...
         * @param prefixName
         */
        setTemplateInfo(modelType, modelName, prefixName, parentModelType = undefined) {
            if (parentModelType == undefined) {
                parentModelType = modelType;
            }

            console.log('setTemplateInfo modelType', modelType);
            console.log('setTemplateInfo parentModelType', parentModelType);
            this.templateInfo.modelType = modelType;
            this.templateInfo.modelName = modelName;
            this.templateInfo.prefixName = prefixName;
            this.templateInfo.parentModelType = parentModelType;
        },
        /**
         * 评分卡-合并tab
         */
        ...
        indexMethed(index) {
            ...
            if (this.elPagination.pageSizes > this.elPagination.pageSize) {
                ...
                return Number(this.elPagination.currentPage) * Number(this.elPagination.pageSizes)  + index - this.pgsz + 1 ;
            }
        },
        /**
         * 单页数据变化监听
        ...
        handleSizeChange(val) {
            ...
            if (val >= elPagination.pageSize) {
                elPagination.currentPage = 1;
            }
            console.log(`sizeChange ${val} 条`);
            console.log('handleSizeChange elPagination', elPagination);
            this.search(this.templateInfo.modelType, this.lastRuleForm, elPagination, this.templateInfo.parentModelType);
            this.pgsz = val
        },
        /**
         * 当前页变化监听
        ...
        formatPZRules(list) {
            const list_1 = [];
            const list_2 = [];
            for (let i = 0; i < list.ruleResultList.length; i++) {
                list_1.push({ rule_name: list.ruleResultList[i].ruleName });
                list_2.push({
                hit_flag_desc: list.ruleResultList[i].hitFlag == "notHit" ? "未命中" : "命中",
                });
            }
        ...
        formatPTRules(data) {
            const list_1 = [];
            const list_2 = [];
            list_1.push({ rule_name: data.ruleName });
        ...
        getDataPF(ruleForm, parentModelType = undefined) {
            return new Promise((resolve, reject) => {
                ...
                .then(async (obj) => {
                    let list = [];
                     this.elPagination.pageSize = 0;
                    if (parentModelType == 'GL') {
                        list = obj.data.field_map;
                         this.elPagination.pageSize = list[0] == undefined ? 0 :list[0].pageParam;
                    } else {
                        list = obj.data;
                         this.elPagination.pageSize = 
                        obj.pageParam == undefined ? 0  : (obj.pageParam.total == undefined ? 0 : obj.pageParam.total);
                    }
                    ...
                    if (list.length == 0) {
                     ...
                     if (parentModelType == 'GL') {
                        for (let i = 0; i < list.length; i++) {
                        list[i].score_result_bak =list[i].score_result; 
            ...
            return new Promise((resolve, reject) => {
                ...
                .then(async (obj) => {
                    ...
                    if (list.length == 0) {
                      ...
                      if (parentModelType == 'GL') {
                        for (let i = 0; i < list.length; i++) {
                        list[i].rule_result_bak = list[i].rule_result; 
        ...
        getDataPZ(ruleForm, parentModelType = undefined) {
            return new Promise((resolve,reject) => {
                ...
                .then(async (obj) => {
                    ...
                    if (parentModelType == 'GL') {
                         for (let i = 0; i < list.length; i++) {
                            list[i].rule_result_bak = list[i].rule_result; // 将模型中字符串 Item转换为object Item
            ...
            return new Promise((resolve,reject) => {
                ...
                .then(async (obj) => {
                    ...
                    if (parentModelType == 'GL') {
                        ...
                        list = obj.data;
                        this.elPagination.pageSize = obj.pageParam == undefined ? 0 : (obj.pageParam.total == undefined ? 0 : obj.pageParam.total);
                    }
                    ...
                    for (let i = 0; i < list.length; i++) {
                        await this.formatGSRules(list[i].children.children);
