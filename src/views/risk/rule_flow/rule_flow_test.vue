     ...
     */
        ...
        openDialog() {
            ...
            that.$dialog.open({
                ...
                title: "规则流代码选择",
                toRequest: {
                    url: "/views/risk/rule_flow/rule_flow_lookup.json",
                    method: "get",
                },
                width: "60%",
            });
        ...
        jsonCallBack(btnConfig, response) {
            ...
            this.form.flowCode = `${response.flow_code}`;
            this.form.flowName = `${response.flow_name}`;
            this.form.version = `${response.curr_version}`;
            this.form.branch_id = `${response.branch_id}`;
            //选择回带触发查询属性方法
            this.selectTreeAttributes(response.flow_code,response.curr_version);
        },
        //查询规则流所需的属性
        selectTreeAttributes(flowCode,version) {
            let params = {
                flowCode: flowCode,
                version:version
            };
            ...
            this.$refs.form.validate((valid) => {
                if (valid) {
                    ...
                    this.$http
                        .invokeAPI("/SUMP/icmcall/icmRPCCall", "POST", {
                            ...params,
                            servicecode: "deci030009"
                        })
                        .then((obj) => {
                            // if (obj.code === "200") {
                            //     console.log("测试接口请求回调", obj);
                            //     this.responseMessage = obj.data.field_map;
                            //     this.modifyDialogVisible = true;
                         if (obj.code === "200") { 
                                    that.$dialog.open({
                                        that: that,
                                        title: "测试成功，命中节点响应数据如下：",
                                        toRequest: {
                                            url: "@FlowCodeTest",
                                            method: "get",
                                            params: {
                                                flow_code: this.form.flowCode,
                                                flow_name: this.form.flowName,
                                                reponse:obj.data.field_map
                                            },
                                        },
