      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="3">
          ...
          <el-form :model="exceptForm" :rules="exceptRule" ref="exceptForm" class="search-form">
            <el-row v-for="(item, index) in exceptForm.locfnos" :key="index.fileType" >
              <el-col :span="3">
                <el-form-item :label="item.fileType" :prop="'locfnos.'+index+'.fileType'"></el-form-item>
                <!-- <el-form-item  :prop="'locfnos.'+index+'.abnormal_type'" align="center">
                  {{getExceptTypeName(item.abnormal_type)}}
                </el-form-item> -->
              </el-col>
              <el-col :span="6">
                <el-form-item>
                  <el-radio-group v-model="item.isornot">
                    ...
                    <el-radio label="2" :disabled="disableState">解除异常</el-radio>
                  </el-radio-group>
                </el-form-item>
              </el-col>
              <el-col :span="13">
                <el-form-item
                  label="附件上传"
                  :prop="'locfnos.'+index+'.credential'"
                  :rules="rules.credential"
                >
                  <el-upload
                    :on-change="attachmentChange.bind(this, item,index)"
                    :on-remove="attachmentRemove.bind(this, item,index)"
                    :limit="5"
                    :file-list="exceptForm.exceptFileList"
                    :auto-upload="false"
                ...
                <el-form-item label="附件信息">
                  <el-row v-for="(ele, key) in item.buctetKey.split(',')" :key="key">
                    ...
                    <!-- <el-col :span="4">
                      <el-button type="primary" size="small" @click="deleteClick(item.buctetKey.split(','), ele, item.fileName.split('\b')[key],key)">{{$i18ns('删除')}}</el-button>
                    </el-col> -->
                  </el-row>
                </el-form-item>
              </el-col>
              ...
              <!-- <el-form-item :prop="'locfnos.'+index+'.file_name'" :rules="rules.file_name"></el-form-item> -->
            </el-row>
...
export default {
  ...
  methods: {
    ...
    getReviewData() {
      let params = {
        servicecode: "PLMSKGTS0095",
        // applid: this.cParentParams.appl_id //申请编号
    ...
    attachmentChange(item, idx, file, fileList) {
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/file", "post", formData)
        .then(res => {
          if (res && res.code === "200" && res.data) {
            file.status = "success";
            item.credential.push({"path":res.data.path,"file_name":file.name})
            // item.credential.push(res.data.path);
            // item.file_name.push(file.name);
     ...
     * 删除历史文件
     */
    deleteClick(item,bucket,path,fileName,key) {
       let index = null;
      item.bucket((item, key) => {
        if (item.uid === bucket.uid) {
          index = key;
          return false;
        }
      });
      if (fileList.length === 0) {
        console.log(this);
        this.$refs["exceptForm"].validateField("locfnos."+idx+".item.bucket");
      }
      // bucket.splice(key,1)
    },
    /**
    ...
    submit(exceptForm) {
      this.$refs[exceptForm].validate(valid => {
        if (valid) {
          console.log(this.exceptForm.locfnos);
          let params = {
            servicecode: "PLMSKGS10689",
            locfnos: this.exceptForm.locfnos,
            lncatp:"ABNORMAL",
            idtftp:"101",
            custid:this.cParentParams.custno,
            idtfno:this.cParentParams.idtfno,
            custna:this.cParentParams.acctna,
