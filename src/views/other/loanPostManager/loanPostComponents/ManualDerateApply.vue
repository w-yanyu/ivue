    ...
    <el-collapse v-model="activeNasmes" class="custom-common--collapse">
      <el-collapse-item name="first">
        ...
        <el-form  ref="basicForm" :model="basicForm" label-width="120px" class="custom-common--form">
          <el-row>
            <el-col :span="6">
              <el-form-item label="借据号123" prop="lncfno" :rules="[{ required: true, message: '请输入借据号'}]">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="减免总金额">
                <el-input v-model="adjustForm.dtsfam" ></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
...
export default {
  ...
  methods: {
    ...
    pageRestore() {
      ...
      this.debtForm.acctno = "";
      this.debtForm.lnreamTotal = "";
      this.debtForm.lnrebl = "";
      this.debtForm.clssst = "";
      this.debtForm.ovdays = "";
      this.debtForm.lnnpbl = "";
      this.debtForm.lnopbl = "";
      this.debtForm.lndpbl = "";
      this.debtForm.hxxxpr = "";
      this.debtForm.yjxxni = "";
      this.debtForm.ysxxoiTotal = "";
      this.debtForm.yclixi = "";
      this.debtForm.yjxxpi = "";
      this.debtForm.ysxxpiTotal = "";
      this.debtForm.hxxxinTotal = "";
      this.debtForm.ysxxfe = "";
      this.adjustForm.remthd = "";
      this.adjustForm.dtsfam = "";
      this.adjustForm.jmlixi = "";
      this.adjustForm.jmfaxi = "";
      this.adjustForm.jmznf = "";
      this.adjustForm.urgent = "";
      this.adjustForm.remttp = "";
      this.adjustForm.adjrsn = "";
      this.adjustForm.khjmsqsj = "";
      this.adjustForm.yyjmyxq = "";
    ...
    lncfnoBlur() {
      ...
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "POST", params)
        .then(response => {
          if (response && response.code === "200" && response.data && response.data.customerLoan && response.data.redupara) {
            let cus = response.data.customerLoan;
            let redupara = response.data.redupara;
            // 基础信息填充
            this.basicForm.custno = cus.custno;
            this.basicForm.acctna = cus.acctna;
            this.basicForm.prodna = cus.prodna;
            this.basicForm.idtfno = cus.idtfno;
            this.basicForm.prodcd = cus.prodcd;
            this.basicForm.phonno = cus.phonno;
            // 欠款信息填充
            this.debtForm.acctno =  cus.acctno;
            this.debtForm.lnreamTotal = this.sumCalc(cus.lnream, cus.yclixi); // 逾期金额（总）= 逾期金额 + 逾期应计利息
            this.debtForm.lnrebl = cus.lnrebl;
            this.debtForm.clssst = cus.clssst;
            this.debtForm.ovdays = cus.ovdays;
            this.debtForm.lnnpbl = cus.lnnpbl;
            this.debtForm.lnopbl = cus.lnopbl;
            this.debtForm.lndpbl = cus.lndpbl;
            this.debtForm.hxxxpr = cus.hxxxpr;
            this.debtForm.yjxxni = this.sumCalc(cus.ysyjni, cus.csyjni); // 应计利息 = 应收应计利息 + 催收应计利息
            this.debtForm.ysxxoiTotal = this.sumCalc(cus.ysxxoi, cus.csxxoi); // 应收欠息（总） = 应收应计利息 + 催收欠息
            this.debtForm.yclixi = Number(cus.yclixi).toFixed(2);
            this.debtForm.yjxxpi = this.sumCalc(cus.ysyjpi, cus.csyjpi); // 应计罚息 = 应收应计罚息 + 催收应计罚息
            this.debtForm.ysxxpiTotal = this.sumCalc(cus.ysxxpi, cus.csxxpi); // 应收罚息(总) = 应收罚息 + 催收罚息
            this.debtForm.hxxxinTotal = this.sumCalc(cus.hxxxin, cus.hxxxoi, cus.hxprin); // 核销利息(总) = 核销利息隐藏 + 核销欠息 + 核销本金利息
            this.debtForm.ysxxfe = cus.ysxxfe;
            // 调整信息填充
            // 减免利息 = 应收欠息 + 催收欠息 + 逾期应计利息 + 核销利息 + 核销欠息 + 核销本金利息
            this.adjustForm.jmlixi = this.sumCalc(cus.ysxxoi, cus.csxxoi, cus.yclixi, cus.hxxxin, cus.hxxxoi, cus.hxprin);
            // 减免罚息 = 应收罚息 + 催收罚息 + 应收应计罚息 + 催收应计罚息
            this.adjustForm.jmfaxi = this.sumCalc(cus.ysxxpi, cus.csxxpi, cus.ysyjpi, cus.csyjpi);
            this.adjustForm.jmznf = cus.ysxxfe;
            this.adjustForm.dtsfam = this.sumCalc(this.adjustForm.jmlixi, this.adjustForm.jmfaxi, this.adjustForm.jmznf);
            this.adjustForm.yyjmyxqDisableDate = cus.yyjmyxq;
            this.adjustForm.ovdate = cus.ovdate;
            this.adjustForm.jzdate = cus.jzdate;
            // 减免配置信息填充
            this.reduceParam.jmlixi = this.adjustForm.jmlixi; // 减免利息最大值记录
            this.reduceParam.jmfaxi = this.adjustForm.jmfaxi; // 减免罚息最大值记录
            this.reduceParam.jmznf = this.adjustForm.jmznf; // 减免逾期违约金最大值记录
    ...
    submit() {
      ...
      Promise.all(validateList)
        .then(res => {
          let params = {
            servicecode: "AS_PLMS_KG_RGJMFLOW",
