              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="凭证类型" :prop="`ticketList[${index}].ticketType`">
                    <el-select v-model="applyForm.ticketList[index].ticketType" placeholder="请选择凭证类型" :disabled="false">
                      <el-option
                ...
                <el-col :span="12">
                  <el-form-item label="总金额" :prop="`ticketList[${index}].ticketSum`">
                    <el-input v-model="applyForm.ticketList[index].ticketSum" :disabled="false"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
...
export default {
  name: "APPHandleLoanPurposeControl",
  props: {
      ...
      this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200" && res.data) {
            ...
            // 基本信息带回
            this.basicForm.customerAccount = response.lncfno;  //借据号
            this.basicForm.customerId = response.idtfno;       //身份证号
            this.basicForm.mobileNo = response.phoneno;     //手机号
            this.basicForm.customerName = response.custna;  //客户号
            this.basicForm.productChineseName = response.prodna;  //产品号
            this.basicForm.accountStatus = response.clssst;   //贷款形态
            this.basicForm.loanUse = response.loan_purpose;  //贷款用途
            this.basicForm.loanLimit = response.loan_amt;   //放款金额
            this.basicForm.signContractAT = response.sign_contractat;  //签订合同附件
...
            };
          ...
          if (res && res.code === "200" && res.data) {
            ...
            for (let i = 0 ; i < gslxx.fileName.split("\\b").length; i++) {
              const map = {
                ...
                file_name: gslxx.fileName.split("\\b")[i],
                path: gslxx.filePath.split("\\b")[i],
                status: "success",
              }
              this.aaumUploadFileList.push(map);
              this.voucherForm.aaumUploadList.push(map);
              }
              }
             }
    ...
    /**
     * 根据申请号获取凭证信息
     * @param fileList 上传凭证文件信息   11111111111
     */
    getVoucherInfo() {
      let params = {
          ...
          if (res && res.code === "200" && res.data) {
            ...
            for (let i = 0; i < ticketList.length; i++) {
              const ele = {
                ...
                ticketInterval: [ticketList[i].pzstart_date, ticketList[i].pzend_date],
              ...
              for (let index = 0; index < fileList.length; index++) {
                ...
                for (let j = 0; j < element.fileName.split("\\b").length; j++) {
                  const map = {
                    ...
                    file_name: element.fileName.split("\\b")[j],
                    path: element.filePath.split("\\b")[j],
                    status: "success",
                  }
                  ele.ticketAccessoryList.push(map);
                }
              }
              console.log( this.applyForm.ticketList)
      ...
      formData.append("org_id", userInfo.org_id);
        ...
        .then(res => {
          if (res && res.code === "200" && res.data && res.data.path) {
            ...
            this.applyForm.ticketList[index].ticketAccessory.push(file);
    ...
    submit() {
      ...
      Promise.all(this.validateList).then(res => {
        ...
        this.applyForm.ticketList.forEach(item => {
          let map = {
           ...
           // ticketList: item.ticketAccessory,
            ticketList: item.ticketAccessoryList,
          }
        ...
        if(!this.moneysum()){
          this.$message({
            type: "warning",
            message: "请输入正确的总金额，总金额必须大于放款金额的80%"
          });
          return;
        }
        this.$http.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
