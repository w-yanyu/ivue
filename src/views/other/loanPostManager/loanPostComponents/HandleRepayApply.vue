<template>
  <div>
    ...
    <div class="custom-common--footer">
      <el-button type="primary" size="small" @click="checkAll('save')" plain>{{$i18ns('保存')}}</el-button>
  ...
  data() {
    return {
      ...
      repayForm: {
        ...
        adjrs: "",
        remark: "",
        issaved: false //还款申请经办岗信息修改后是否保存
      },
      otherInfoForm: {
        rpacno: "", //默认还款账号
        derana: "", //默认还款账户户名
  ...
  methods: {
    ...
    sfjmtqhk() {
      if (this.repayForm.sfjmtqhk) {
        ...
        this.ifshowAttach = false; //显示四个附加
        // 附件是否必传
    ...
    repayFormMeth() {
      let params = {
        pageName: "HandleRepayApply",
        issaved: this.repayForm.issaved
      };
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200") {
            ...
            if (data.pboprtp === "2") {
              this.ifShowSfjmtqhk = true; //提前还款试算，
    ...
    clearRepayHis() {
      ...
      this.repayForm.sfzxqx = false; //初始化值--重置值
      this.repayForm.sfjief = false;
      this.personPayshow = false;
      this.ifwrite = true;
      ...
      MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
        .then(res => {
          if (res && res.code === "200") {
            ...
            this.repayForm.issaved = true; //修改后是否保存
            let msg =
              operation == "save"
                ? "保存成功"
    ...
    copyContent(val) {
      ...
      input.value = val === "0" ? this.earPayAmount : this.jmtqhkje; // 设置内容 需要复制的内容
