        ...
        this.$http.invokeAPI(init_url, "post", init_req)
            .then(response => {
              ...
              if(response.code="200"){
                console.log('us0604======>',response)
                let responseData = response.data;
                console.log("组织架构解析前数据：",responseData);
                for(let i in responseData){
                  if(responseData[i].pid === "-1"){
                    ...
                    responseData[i].shareholding_ratio ="持股比例"+responseData[i].shareholding_ratio+"%"
                    responseData[i].is_core_subcompany =responseData[i].is_core_subcompany==='1'?'母公司':''
                  }else if(responseData[i].id === this.cParentParams.cust_no){
              ...
              console.log("组织架构解析后数据：",responseData);
              /* this.mygoChartData = [
            ...
            for(let i in this.mygoChartData){
                ...
                mySelf.myDiagram.nodeTemplateMap.add("mainNode",
                  MAKE(go.Node,new go.Binding("location", "loc", go.Point.parse),
                    ...
                    MAKE(go.Panel, "Horizontal",{padding:5},
                      ...
                      MAKE(go.Panel, "Vertical",
                      MAKE(go.TextBlock, {margin: new go.Margin(10, 0, 0, 15), stroke: "#4f5256", font: "bold 14px sans-serif",},new go.Binding("text", "cust_name")),
                ...
                mySelf.myDiagram.nodeTemplateMap.add("mainNode2",
                  MAKE(go.Node,new go.Binding("location", "loc", go.Point.parse),
                    ...
                    MAKE(go.Panel, "Horizontal",{padding:5},
                      ...
                      MAKE(go.Panel, "Vertical",
                      MAKE(go.TextBlock, {margin: new go.Margin(10, 0, 0, 15), stroke: "#4f5256", font: "bold 14px sans-serif",},new go.Binding("text", "cust_name")),
                      MAKE(go.TextBlock, {margin: new go.Margin(0, 0, 0, 15), stroke: "#4f5256", font: "bold 14px sans-serif",},new go.Binding("text", "shareholding_ratio")),
                      MAKE(go.TextBlock, {margin: new go.Margin(0, 0, 0, 15), stroke: "#4f5256", font: "bold 14px sans-serif",},new go.Binding("text", "is_core_subcompany")),
