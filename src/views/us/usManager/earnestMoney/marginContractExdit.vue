        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="合同金额" prop="value_price" label-width="120px">
              <el-input class="input_el" v-model.number="ruleForm.value_price" @blur="guarBaseAmtInput($event)"   type="number">
              </el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="折算基准币种" prop="base_amt" label-width="120px">
              <el-input class="input_el" v-model.number="ruleForm.base_amt" @blur="guarBaseAmtInput($event)"    type="number">
              </el-input>
            </el-form-item>
          </el-col>
  ...
  export default {
    ...
    methods: {
      initDefineData() {
        ...
        that.ruleForm.sign_dt = that.cParentParams.sign_dt.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3"); //合同签订日期
        that.ruleForm.start_date = that.cParentParams.start_date.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3"); //合同起始日期
        that.ruleForm.end_date = that.cParentParams.end_date.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3"); //合同到期日期
      ...
      initData() {
        ...
        request_map["servicecode"] = "ICMSLNTS5739";
        request_map["appl_id"] = that.cParentParams.appl_id;
        request_map["margin_ctr_ppsl_id"] = that.cParentParams.margin_ctr_ppsl_id;
        this.$http
          .invokeAPI(request_url, "post", request_map)
          .then((response) => {
            ...
            if (response.code === "200") {
              ...
              for (let i in that.tableData) {
                if (
                  that.tableData[i].margin_ctr_ppsl_id !== "" &&
                  that.tableData[i].margin_ctr_ppsl_id !== null
                ) {
                  console.log(
                    "margin_ctr_ppsl_id",
                    that.tableData[i].margin_ctr_ppsl_id
      ...
      clickConfirm() {
        ...
        request_map["servicecode"] = "ICMSLNTS5711";
