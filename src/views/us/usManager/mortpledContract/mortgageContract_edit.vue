    ...
    <div class="menu-form--container">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="130px" class="custom-common--form">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="合同金额(元)" label-width="195px" prop="value_price">
  ...
  export default {
    ...
    methods: {
      ...
      initDefineData() {
        ...
        that.ruleForm.sec_cust_no = that.cParentParams.sec_cust_no; //抵质押人
        that.ruleForm.dispute_rslt = that.cParentParams.dispute_rslt; //纠纷解决方式
        that.ruleForm.value_ccy = that.cParentParams.value_ccy; //币种
        that.ruleForm.base_ccy_cd = that.cParentParams.base_ccy_cd; //基准币种
        that.ruleForm.exc_rate = that.cParentParams.exc_rate; //折算汇率
        that.ruleForm.value_price = that.cParentParams.value_price; //合同金额
        that.ruleForm.value_base_amt = that.cParentParams.value_price; //合同金额(折算基准币种)
        that.ruleForm.part_out_flag = that.cParentParams.part_out_flag; //是否允许押品部分出库
        that.ruleForm.sec_occupy_amt = that.cParentParams.sec_occupy_amt; //本次使用金额
        that.ruleForm.base_amt = that.cParentParams.sec_occupy_amt; //本次使用金额(折算基准币种)
        that.ruleForm.mort_type = that.cParentParams.mort_type; //抵押担保类型
        ...
        that.ruleForm.sign_dt = that.cParentParams.sign_dt.replace(
          /^(\d{4})(\d{2})(\d{2})$/,
          "$1-$2-$3"
        ); //合同签订日期
        that.ruleForm.start_date = that.cParentParams.start_date.replace(
          /^(\d{4})(\d{2})(\d{2})$/,
          "$1-$2-$3"
        ); //合同起始日期
        that.ruleForm.end_date = that.cParentParams.end_date.replace(
          /^(\d{4})(\d{2})(\d{2})$/,
          "$1-$2-$3"
        ); //合同到期日期
      },
          ...
          .then((response) => {
            ...
            if (response.code === "200") {
              ...
              for (let i in that.tableData) {
                if (
                  that.tableData[i].colt_ctr_ppsl_id !== "" &&
                  that.tableData[i].colt_ctr_ppsl_id !== null
                ) {
                  console.log(
                    "colt_ctr_ppsl_id",
                    that.tableData[i].colt_ctr_ppsl_id
      ...
      clickConfirm() {
        ...
        request_map["servicecode"] = "ICMSLNTS5001";
        request_map["appl_id"] = that.cParentParams.appl_id;
        request_map["colt_ctr_ppsl_id"] = that.cParentParams.colt_ctr_ppsl_id;
        request_map["sec_type"] = that.cParentParams.sec_type;
        ...
        request_map["value_base_amt"] = that.ruleForm.value_base_amt;
