<template>
  <div class="page_content">
    <table border="1" class="horizontal_info">
      <tr >
        <td class="width12">{{$i18ns('客户编号：')}}</td>
        <td class="width12">{{ cParentParams.cust_no }}</td>
        <td class="width12">{{$i18ns('报表行业类型')}}</td>
        <td class="width12">{{ indu_type }}</td>
        <td class="width12">{{$i18ns('报表模板：')}}</td>
        <td class="width12">{{ rpt_type }}</td>
        <td class="width12">{{$i18ns('报表日期：')}}</td>
        <td class="width12">{{ cParentParams.rpt_date }}</td>
      </tr>

      <tr >
        <td>{{$i18ns('财务报表合并口径')}}</td>
        <td>
          <div>
            <el-select
              v-model="unit_way"
        ...
        <td><span>*</span>{{$i18ns('审计情况')}}</td>
        <td>
          <div>
            <el-select
              v-model="is_check_flag"
              :placeholder="$i18ns('请选择...')"
              class="table_select"
              disabled
            >
              <el-option
                v-for="item in checkFlagType"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
        ...
        <td>
          <div>
            <el-upload
              ...
              :auto-upload="false"
            >
              <el-button  type="success" class="excel-btn " style="width:100%" >{{$i18ns('Excel导入')}}</el-button>
            </el-upload>
          </div>
        </td>
      </tr>
    ...
    <table border="1" class="horizontal_title">
      <tr >
        <td class="width30">{{ indu_type }}</td>
        <td class="width30"> {{$i18ns('日期')}}{{ cParentParams.rpt_date }}</td>
        <td class="width30">{{$i18ns('单位：元')}}</td>
      </tr>
      </table>

    <table class="tabel_balance">
      <tr class="tr_title">
        <th class="th_big">{{$i18ns('资产')}}</th>
        <th class="th_small">{{$i18ns('行次')}}</th>
        <th class="th_big">{{$i18ns('上期金额')}}</th>
        <th class="th_big">{{$i18ns('本期金额')}}</th>
        <th class="th_big">{{$i18ns('负债和所有者权益')}}</th>
        <th class="th_small">{{$i18ns('行次')}}</th>
        <th class="th_big">{{$i18ns('上期金额')}}</th>
        <th class="th_big">{{$i18ns('本期金额')}}</th>
      </tr>
      <tr
        ...
        :class="{
          'link-item-active': itemData.id == activeLinkId,
          'link-item-hover': index == hoverIndex
        }"
        :key="index"
        @mouseover="hoverIndex = index"
        @mouseout="hoverIndex = -1"
      >
        <td class="textAginleft" v-if="itemData.l_alias_name">
          {{ itemData.l_alias_name }}({{ itemData.l_item_code }})
        </td>
        <td v-if="itemData.l_alias_name">{{ itemData.l_rowno }}</td>
        <td v-if="itemData.l_alias_name">
          <!--  <el-input class="table_content_input" v-model="itemData.l_begin" :disabled="true">
          </el-input> -->
          <el-input-number
            class="table_content_input"
            v-model="itemData.l_begin"
            :disabled="true"
            :precision="2"
          >
          </el-input-number>
        ...
        <td v-if="itemData.l_alias_name">
          <!--  <el-input class="table_content_input" type='number' v-model="itemData.l_end"
            :disabled="itemData.l_form_ula!=''" @blur="itemLInput(itemData)">
          </el-input> -->
          <el-input-number
            class="table_content_input"
            v-model="itemData.l_end"
            :disabled="itemData.l_form_ula != ''"
            :precision="2"
          ></el-input-number>
        </td>
        <td v-if="itemData.r_alias_name" class="textAginleft">
          {{ itemData.r_alias_name }}({{ itemData.r_item_code }})
        </td>
        <td v-if="itemData.r_alias_name">{{ itemData.r_rowno }}</td>
        <td v-if="itemData.r_alias_name">
          <!-- <el-input class="table_content_input" v-model="itemData.r_begin" :disabled="true">
          </el-input> -->
          <el-input-number
            class="table_content_input"
            v-model="itemData.r_begin"
            :disabled="true"
            :precision="2"
          >
          </el-input-number>
        ...
        <td v-if="itemData.r_alias_name">
          <!-- <el-input class="table_content_input" type='number' v-model="itemData.r_end"
            :disabled="itemData.r_form_ula!=''" @blur="itemRInput(itemData)">
          </el-input> -->
          <el-input-number
            class="table_content_input"
            v-model="itemData.r_end"
            :disabled="itemData.r_form_ula != ''"
            :precision="2"
          ></el-input-number>
        </td>
      </tr>
    ...
    <div>
      ...
      <el-button class="submit" type="info" plain @click.native.prevent="close"
        >{{$i18ns('返回')}}</el-button
      >
      <!-- <el-button class="submit" type="info" plain @click.native.prevent="test">{{$i18ns('测试')}}跳转</el-button> -->
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      isShowSaveBtn: false, //是否显示存储按钮
      formulaData: [], //含有公式的数组
      importUrl:
        "/views/us/usManager/corporateCustomerFinancialStatements/importUrl.json",
      isImportTbale: "0" //是否是表格导入的数据
    };
  },
  mounted() {
    let that = this;
    ...
    that.is_check_flag = this.cParentParams.is_check_flag;
    that.initData();
  },
  methods: {
    initData() {
      ...
      request_map["servicecode"] = "us0934";
    ...
    itemLInput(item) {
      ...
      if (item.l_item_code !== "") {
        let modifyArray = that.seachMachArry(item.l_item_code, item.l_end);
        console.log("需要修改的数组", modifyArray);
        that.$nextTick(() => {
          for (let i in modifyArray) {
            if (modifyArray[i].side === "l") {
              that.tableData[modifyArray[i].index].l_end =
                modifyArray[i].totalValue;
            } else {
              that.tableData[modifyArray[i].index].r_end =
                modifyArray[i].totalValue;
            }
          }
        });
    ...
    itemRInput(item) {
      let that = this;
      let modifyArray = [];
      console.log("右边输入框输入", item);
      // item.r_end = parseFloat(item.r_end)
      if (item.r_item_code !== "") {
        let modifyArray = that.seachMachArry(item.r_item_code, item.r_end);
        console.log(
          '  console.log("需要修改的数组", that.tableData[modifyArray[i].index]);',
          modifyArray
        );
        that.$nextTick(() => {
          for (let i in modifyArray) {
      ...
      that.$nextTick(() => {
        let value = that.fmtMoney(item.r_end);
        item.r_end = value;
      });
    },
    seachMachArry(key, value) {
      ...
      for (let i in that.formulaData) {
        if (that.formulaData[i][key] !== undefined) {
          ...
          let totalValue = "";
          totalValue = that.isinclude(
            formulaStr,
            "e",
            sourcelist[0],
            that.formulaData[i]
          ); //本期金额，促发公式//sourcelist[0]为等号右边
          let modifyObj = {
    ...
    rptPerdTpFormat(data) {
      switch (data.rpt_perd_type) {
        case "D":
          return this.$i18ns('日报');
        case "M":
          return this.$i18ns('月报');
        case "Q":
          return this.$i18ns('季报');
        case "S":
          return this.$i18ns('半年报');
        case "Y":
          return this.$i18ns('年报');
      }
    },
    //
    unitWayFormat(data) {
      return data.unit_way === "1"
        ? this.$i18ns('汇总')
        : data.unit_way === "2"
        ? this.$i18ns('单一')
        : this.$i18ns('合并');
    ...
    checkFlagFormat(data) {
      return data.is_check_flag === "0" ? '否' : '是';
    },
    //关闭对话框
    close() {
      this.$dialog.close();
    ...
    importfxx(obj) {
      ...
      FileReader.prototype.readAsBinaryString = function(f) {
        ...
        reader.onload = function(e) {
          ...
          console.log("获取对象key===", Object.keys(outdata[0]));
          // outdata就是读取的数据（不包含标题行即表头，表头会作为对象的下标）
          // 此处可对数据进行处理
          if (Object.keys(outdata[0])[0] !== "资产负债表") {
          ...
          outdata.map(v => {
            ...
            // obj.r_alias_na = v['负债和损益科目名称']
            // obj.r_rowno = v['行号_1']
            // obj.r_item_cd = v['科目号_1']
            // obj.r_begin = v['年初数_1']
            // obj.r_end = v['期末数_1']
            // obj.r_form_ula = ""

            //Excel没有删掉目录的接触
            obj.l_alias_name = v["资产负债表"];
            obj.l_rowno = v["__EMPTY"];
            obj.l_item_code = v["__EMPTY_1"];
            obj.l_begin = _this.fmtMoney(v["__EMPTY_2"]);
            obj.l_end = _this.fmtMoney(v["__EMPTY_3"]);
            obj.l_form_ula = "";

            obj.r_alias_name = v["__EMPTY_4"];
            obj.r_rowno = v["__EMPTY_5"];
            obj.r_item_code = v["__EMPTY_6"];
            obj.r_begin = _this.fmtMoney(v["__EMPTY_7"]);
            obj.r_end = _this.fmtMoney(v["__EMPTY_8"]);
            obj.r_form_ula = "";
