    ...
    <div class="container-content">
      ...
      <div class="container-centent--table">
        <el-table ref="multipleTable" :data="tableData" :border="true" style="width: 100%" stripe >
          <el-table-column label="最新调整申请编号" prop="applid" align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="业绩归属" prop="branch_name" width="500" align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="业绩归属(隐藏)" v-if="false" prop="performance_attr" width="500" align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="产品大类" prop="prodTypeName"  align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="产品大类"  v-if="false" prop="prod_type"  align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="账龄" prop="acct_duration"  align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="最小回收率%(不包含)" prop="min_recovery_rate"  align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="最大回收率%(包含)" prop="max_recovery_rate"  align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="佣金上限%" prop="comm_rate"  align="center" show-overflow-tooltip></el-table-column>
        </el-table>
      </div>
    </div>

    <el-dialog title="规则调整" :visible.sync="strategyModelVisible" :show-close="true" :before-close="handleClose" width="1200px" :append-to-body="true">
      <div class="el-dialog-dev" >
        <div class="container-content">
          <div class="container-content--search">
            <el-button type="primary" @click="addLast()" plain>新增总部佣金规则</el-button>
          </div>
          <div class="container-centent--table">
            <el-table ref="modifyTable" :data="tableModifyData" :border="true" stripe >
              <el-table-column label="业绩归属" prop="performance_attr" align="center" show-overflow-tooltip>
                <template slot-scope="scope">
                  <el-select v-model="scope.row.performance_attr" v-if="scope.row.status"  placeholder="请选择机构">
              ...
              <el-table-column label="佣金上限%" prop="comm_rate" width="130" align="center">
                <template slot-scope="scope">
                  <el-input-number v-model="scope.row.comm_rate" :min="0" :max="100" label="佣金上限"  v-if="scope.row.status"></el-input-number>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      queryHeadStrategy(){
        let params2 = {
          servicecode: "RLMSPLTS1206_pl1206",
        }
        MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params2).then(response => {
          if (response && response.code === "200" ) {
            response.data.forEach(item=>{
              ...
              strategy.branch_name = this.getDictName(item.performance_attr,this.branchIdList);
              strategy.performance_attr =  item.performance_attr;
      ...
      submit(){
        ...
        this.tableModifyData.forEach(item=>{
          if(item.status==true){
            existModify = true;
          }
        })
      ...
      requestRlms(){
        let params2 = {
          servicecode: "RLMSPLTS1201_pl1201",
          headCommRulesList: this.tableModifyData,
        }
        MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params2).then(response => {
          if (response && response.code === "200" ) {
            this.tableModifyData = [];
      ...
      saveRow(index,row){
        ...
        if(!row.performance_attr || row.performance_attr.length===0){
        ...
        if(!row.comm_rate || row.comm_rate.length===0){
          this.$message({type: "warning", message: "请输入佣金上限！"});
          return;
        }
        row.branch_name =this.getDictName(row.performance_attr,this.branchIdList) ;
