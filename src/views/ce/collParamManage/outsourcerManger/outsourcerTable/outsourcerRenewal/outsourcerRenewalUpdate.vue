            ...
            <el-row>
              <el-col :span="12">
                <el-form-item class="require-asterisk" label="续约材料" prop="xycl">
                  <upload-file ref="xyclProp" :uploadFileList="appForm.xycl" :uploadDisabled="false" @changeFile="xyclChange" :key="timer"></upload-file>
                </el-form-item>
              </el-col>
              <el-col :span="12">
...
<script>
  ...
  export default {
    ...
    methods: {
      initRule(){
        this.appRules = {
          startDate: [{required: true,  message: "请输入续约开始时间"}],
          endDate: [{required: true,  message: "请输入续约结束时间"}],
          xycl:[{ validator: this.xyclValidate, trigger: 'change' }],
        }
      },
      /**
       * 获取数据字典列表
        ...
        this.$refs.fileForm.validate(valid => {
          if (valid) {
            this.$confirm("是否保存申请?", "提示", {
              ...
              let params = {
                servicecode: "RLMSPLTS1019",
                applid: this.cParentParams.applid,
                startDate:this.appForm.startDate,   //续签开始时间
                endDate:this.appForm.endDate,   //续签结束时间
                xycl:this.appForm.xycl,//续约材料
                qtcl:this.appForm.qtcl,//其他材料
      ...
      clearData(){
        ...
        this.$refs.xyclProp.clearFileList();
       ...
       */
      xyclChange(val){this.appForm.xycl= val;},
      qtclChange(val){this.appForm.qtcl= val;},

      xyclValidate(value, rule, callback) {if (this.appForm.xycl.length === 0) {  return callback(new Error("请上传续约材料"));   }return callback();   },
    }
  }
