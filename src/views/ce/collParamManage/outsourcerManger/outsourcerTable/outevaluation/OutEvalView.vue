...
<!--          </el-col>-->
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item  prop="evaluate_year" label="评价年份" label-width="120px">
              <el-date-picker
          ...
          <el-col :span="8">
            <el-form-item  prop="evaluate_result" label="评价结果" label-width="120px">
              <el-radio-group v-model="evalForm.evaluate_result">
                <el-radio
                v-for="(item,i) in yesOrNoList"
                :key="i" :label="item.dictId"
                @change="radioChange"
                :checked="appForm.evaluate_result === item.dictId"
                :disabled="true"
        ...
        <el-row>
          <!-- <el-col :span="8">
            <el-form-item  label="上传采购项目后评价表" prop="efaprp">
              <upload-file ref="efaprpProp" :uploadFileList="evalForm.efaprp" :uploadDisabled="true" @changeFile="efaprpChange" :key="timer"></upload-file>
            </el-form-item>
          </el-col> -->
          <el-col :span="8">
            <el-form-item  label="外包项目后评价表" prop="popef">
              <upload-file ref="popefProp" :uploadFileList="evalForm.popef" :uploadDisabled="true" @changeFile="popefChange" :key="timer"></upload-file>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$i18ns('备注:')" prop="remark">
              <el-input type="textarea" v-model="evalForm.remark" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.getDictList("E_EVALRESULT", "yesOrNoList"); // 是否
      this.getEvalInfo();
    },
    ...
    methods: {
      initRule(){
        ...
        this.evalRules = {
            ...
            evaluate_result:[{ required: true, message: "请选择评价结果" }],
        }
      },
      /**
       * 获取数据字典列表
      ...
      getBankInfo(){
        let params = {
          ...
          oaOrgId: this.evalForm.oa_org_id,
        }
        MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
          .then(res => {
            if (res && res.code === "200" && res.data) {
      ...
      getEvalInfo(){
        let params = {
          servicecode: "RLMSPLTS1112",
          applid: this.cParentParams.applid,
        }
        MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
          .then(res => {
            if (res && res.code === "200" && res.data) {
              this.evalForm.evaluate_result = res.data.applyInfos.evaluate_result; //评价结果
              this.evalForm.oa_org_id = res.data.applyInfos.oa_org_id; //外包商机构编号
              this.evalForm.evaluate_year = res.data.applyInfos.evaluate_year; //评价年份
              // this.evalForm.efaprp=res.data.sccgs; //上传采购项目后评价表
              this.evalForm.popef = res.data.scwbs; //上传外包项目后评价表
              this.evalForm.remark = res.data.applyInfos.remark; //备注
              this.getBankInfo();
              this.getAppInfo();
              this.timer = new Date().getTime();//更新timer，重新加载子组件
            }
      ...
      radioChange(){
        if(this.evalForm.evaluate_result=='1'){
          this.clearData();
          this.evalForm.evaluate_result = "1";
          this.initRule();
        }
        if(this.evalForm.evaluate_result=='0'){
          this.clearData();
          this.evalForm.evaluate_result = "0";
          this.initRule();
        }
      },
      clearData(){
      ...
      // efaprpValidate(value, rule, callback) {if (this.evalForm.efaprp.length === 0) {  return callback(new Error("上传采购项目后评价表"));   }return callback();   },
      // popefValidate(value, rule, callback) {if (this.evalForm.popef.length === 0) {  return callback(new Error("上传外包项目后评价表"));   }return callback();   },
    }
