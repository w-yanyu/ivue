        ...
        <el-collapse-item name="5">
          ...
          <el-col :span="12">
            <el-form-item class="require-asterisk" label="考核结果:" prop="isWork">
             ...
             <el-row>
              <el-col :span="12">
                <el-form-item class="require-asterisk" label="考核结果" prop="khbg">
                  <upload-file ref="khbgProp" :uploadFileList="appForm.khbg" :uploadDisabled="false" @changeFile="khbgChange" :key="timer"></upload-file>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item class="require-asterisk" label="退出评审单" prop="fileLists" v-show="!isShow">
                  <upload-file ref="fileListsProp" :uploadFileList="appForm.fileLists" :uploadDisabled="false" @changeFile="fileListsChange" :key="timer"></upload-file>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
          <el-col :span="12">
            <el-form-item :class="{'require-sign': !isShow}" :label="$i18ns('备注:')" prop="remakes">
              <el-input type="textarea" v-model="appForm.remakes"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.getDictList("E_ASS_RESULTS", "yesOrNoList"); // 是否
      this.initRule();
      this.getAppInfo();
    },
    ...
    methods: {
       ...
       initRule(){
        this.appRules = {
           isWork: [{required: this.isShow, message: "请选择是否通过"}],
            khbg:[{ validator: this.khbgValidate, trigger: 'change' }],
            fileLists:[{ validator: this.fileListValidate, trigger: 'change' }],
            remakes:[{required: !this.isShow,  message: "请输入备注"}],
        }
      },
      /**
       * 获取数据字典列表
 ...
 radioChange(){
      ...
      getAppInfo(){
        let params = {
          servicecode: "RLMSPLTS1032",
          applid: this.cParentParams.applid,
        }
        MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "post", params)
          .then(res => {
            if (res && res.code === "200" && res.data) {
                this.appForm.isWork = res.data.applyInfos.ass_results,  //是否合作
                this.appForm.oa_org_id = res.data.applyInfos.oa_org_id,
                this.appForm.khbg=res.data.khbg,
                this.appForm.fileLists = res.data.fileLists,//附件
                this.appForm.remakes = res.data.applyInfos.remakes,  //备注
        ...
        this.$refs.appForm.validate(valid => {
          if (valid) {
            this.$confirm("是否确认提交申请?", "提示", {
              ...
              let params = {
                 servicecode: "RLMSPLTS1033",
                 applid:this.appForm.applid,
                  khbg:this.appForm.khbg,
                 ass_results:this.appForm.isWork,  //是否合作
                 oa_org_id: this.appForm.oa_org_id,
                ass_results:this.appForm.isWork,  //是否合作
                fileLists:this.appForm.fileLists,//附件
                remakes:this.appForm.remakes,  //备注
                applid:this.cParentParams.applid,
                lncatp:"SYQPJ",
              }
      ...
      clearData(){
        ...
        this.$refs.khbgProp.clearFileList();
        this.$refs.fileListsProp.clearFileList();
      },
      /**
       * 添加文件回调
       * @param file 当前添加文件对象
       ...
       */
      khbgChange(val){this.appForm.khbg= val;},
      fileListsChange(val){this.appForm.fileLists= val;},
      khbgValidate(value, rule, callback) {if (this.appForm.khbg.length === 0) {  return callback(new Error("请上传考核报告"));   }return callback();   },
      fileListValidate(value, rule, callback) {if (this.appForm.fileLists.length === 0 && !this.isShow) {  return callback(new Error("请上传退出评审单    "));   }return callback();   },
    }
  }
