...
<!--              </el-col>-->
        ...
        <el-collapse-item name="5">
          ...
          <el-form ref="appForms" :rules="appRules" :model="appForms" label-width="200px" class="custom-common--form">
          <el-row>
          <el-col :span="12">
            <el-form-item class="require-asterisk" label="考核结果:" prop="isWorks">
              <el-radio-group v-model="appForms.isWorks">
                <el-radio v-for="(item,i) in yesOrNoList" :key="i" :label="item.dictId" @change="radioChanges" :checked="appForms.isWorks === item.dictId">{{item.dictName}}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
           </el-row>
          <el-row>
          <el-col :span="12">
            <el-form-item class="require-asterisk" label="考核报告" prop="khbg">
              <upload-file ref="kebgProp" :uploadFileList="appForms.khbg" :uploadDisabled="false" @changeFile="khbgChange"></upload-file>
            </el-form-item>
          </el-col>
             <el-col :span="12">
            <el-form-item class="require-asterisk" label="退出评审单" prop="fileLists" v-show="!isShow">
              <upload-file ref="fileListProp" :uploadFileList="appForms.fileLists" :uploadDisabled="false" @changeFile="fileListChange"></upload-file>
             </el-form-item>
          </el-col>
            </el-row>
         <el-row>
          <el-col :span="12">
            <el-form-item  :label="$i18ns('备注:')" prop="remakes">
              <el-input type="textarea" v-model="appForms.remakes"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
          </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      initRule(){
        this.appRules = {
           isWorks: [{required: this.isShow, message: "请选择是否通过"}],
      ...
      getDictList(dictType, listKey) {
       ...
       radioChanges(){
        if(this.appForms.isWorks=='1'){
          this.isShow=true;
          this.clearData();
          this.appForms.isWork = "1";
          this.initRule();
        }
        if(this.appForms.isWorks=='0'){
          this.isShow=false;
          this.clearData();
          this.appForms.isWorks = "0";
          this.initRule();
        }
      },
getCityList() {
        ...
        this.$refs.appForms.validate(valid => {
          if (valid) {
            this.$confirm("是否确认提交申请?", "提示", {
              ...
              let params = {
                 servicecode: "RLMSPLTS1026",
                khbg:this.appForms.khbg,
                 ass_results:this.appForms.isWorks,  //是否合作
                 oa_org_id: this.cParentParams.oa_org_id,
                fileLists:this.appForms.fileLists,//附件
                remakes:this.appForms.remakes,  //备注
                lncatp:"SYQPJ",
              }
      ...
      clearData(){
        ...
        this.$refs.kebgProp.clearFileList();
       ...
       */
      khbgChange(val){this.appForms.khbg= val;},
      fileListChange(val){this.appForms.fileLists= val;},
      khbgValidate(value, rule, callback) {if (this.appForms.khbg.length === 0) {  return callback(new Error("请上传考核报告"));   }return callback();   },
      fileListValidate(value, rule, callback) {if (this.appForms.fileLists.length === 0 && !this.isShow) {  return callback(new Error("请上传退出评审单    "));   }return callback();   },
    }
  }
