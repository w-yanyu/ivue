  ...
  <div class="dialog-box">
    <div class="setting">
      ...
      >
        ...
        <!-- 当卡片为地图时，数据展示信息start -->
        <el-collapse v-model="activeNames" @change="handleChange" v-if="ruleForm.card_i_name === 'cardMap3' || ruleForm.card_i_name === 'cardMap2'">
          <el-collapse-item :title="$i18ns('digitalScreen.map_config_info')" name="1">
            ...
            <el-row>
              <el-col>
                <el-form-item :label="$i18ns('digitalScreen.home_province') + ':'" prop="province">
                  <el-select v-model="merchant.province" :placeholder="$i18ns('digitalScreen.province_choose')" @change="renderCity" clearable>
                    <el-option
            ...
            <el-row>
              <el-col>
                <el-form-item label="" prop="province">
                  <el-select v-model="merchant.city"  :placeholder="$i18ns('digitalScreen.city_choose')" v-if="citySelectAble" clearable>
                    <el-option
            ...
            <el-row>
              <el-col>
                <el-form-item
                  :label="$i18ns('digitalScreen.map_gear') + ':'"
                  prop="gear"
                  class="upload-item"
                >
                  <el-input-number class="input-item" v-model="ruleForm.gear" @change="handleChangeGear" :min="0" :max="5" :step="1"></el-input-number>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col>
                <el-form-item
                  ...
                  class="upload-item"
                >
                  <el-input-number class="input-item" v-model="ruleForm.gear_interval" @change="handleChangeInterval" :min="0" :step="0.1"></el-input-number>
                </el-form-item>
              </el-col>
            </el-row>
          </el-collapse-item>
        ...
        <el-row>
          <el-col>
            <el-form-item :label="$i18ns('机构') + ':'" prop="branch_id">
              <el-select v-model="ruleForm.branch_id" placeholder="请选择" clearable>
                <el-option
...
export default {
  ...
  mounted() {
    ...
    if (this.cParentParams && this.cParentParams.card_id) {
      this.$nextTick(() => {
        this.ruleForm.card_id = this.cParentParams.card_id;
        this.ruleForm.titleName = this.cParentParams.card_name;
        this.ruleForm.unitName = this.cParentParams.card_title;
        this.ruleForm.i = this.cParentParams.card_id;
        this.ruleForm.card_i_name = this.cParentParams.card_i_name;
        this.ruleForm.x = parseInt(this.cParentParams.card_x);
        this.ruleForm.y = parseInt(this.cParentParams.card_y);
        this.ruleForm.w = parseInt(this.cParentParams.card_w);
        this.ruleForm.h = parseInt(this.cParentParams.card_h);
        this.ruleForm.servicecode = this.cParentParams.card_service_code;
        this.ruleForm.scope_range = this.cParentParams.role_id;
        this.ruleForm.scope_range_str = this.cParentParams.card_tags;
        this.ruleForm.card_img = this.cParentParams.card_img;
        this.ruleForm.busi_card_type = this.cParentParams.busi_card_type;
        this.ruleForm.branch_id = this.cParentParams.branch_id || "";
        // 地图配置信息回显
        this.ruleForm.gear = this.cParentParams.gear || "";
        this.ruleForm.gear_interval = this.cParentParams.gear_interval || "";
        this.ruleForm.area1 = this.cParentParams.area1 || "";
        this.ruleForm.area2 = this.cParentParams.area2 || "";
        this.ruleForm.area3 = this.cParentParams.area3 || "";
        this.ruleForm.attribute1 = this.cParentParams.attribute1 || "";
        this.ruleForm.attribute2 = this.cParentParams.attribute2 || "";
        this.ruleForm.attribute3 = this.cParentParams.attribute3 || "";
        this.ruleForm.area_code = this.cParentParams.area_code || "";

        //处理 使用角色 回显数据
        let scopeRangeArray = this.ruleForm.scope_range.split(",");
        let scopeRangeStrArray = this.ruleForm.scope_range_str.split(",");
        if (scopeRangeArray.length === scopeRangeStrArray.length) {
          let temp = [];
          for (let i = 0; i < scopeRangeArray.length; i++) {
            temp.push({
              role_id: scopeRangeArray[i],
              role_name: scopeRangeStrArray[i]
            });
          }
          this.ruleForm.scope_range_tags = temp;
        }
        // 处理回显城市编码对应中文显示
        const citycode = this.cityList.find(item => {item.code === this.ruleForm.area_code})
        if(citycode) {
          this.merchant.city = this.ruleForm.area_code;
        } else {
          this.merchant.province = this.ruleForm.area_code;
        }
        // 获取地图
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          let params = {
            ...
            card_i_type: this.ruleForm.card_id, //卡片种类
            card_i_name: this.ruleForm.card_i_name, //卡片种类
            ...
            area_code: this.merchant.city ? this.merchant.city : this.merchant.province , // 地图编码
          };
          MyAxios.invokeAPI("/SUMP/icmcall/icmRPCCall", "POST", params)
            .then(res => {
