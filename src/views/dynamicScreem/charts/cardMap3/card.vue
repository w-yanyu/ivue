<template>
  <div class="box" ref="box" :key="item.card_id">
...
export default {
  ...
  mounted() {
    ...
    this.$bus.on("refreshMapData", data => {
      data.forEach(ele => {
        if (ele.card_i_name === "cardMap3" && (ele.card_id === this.item.card_id)) {
          this.cityCode = ele.area_code;
          this.cityName = ele.area_code;
          this.getMap(ele);
          // this.getChartData();
        }
      });
    ...
    gearDeal(val) {
      ...
      for (let i = 1; i <= stringToNumber(val.gear); i++) {
        ...
        piecesItem.push({
          ...
          label: (newinterval[i - 1] + "-" + newinterval[i] + this.unitName )|| "", //示例文案
