<template>
  <div>
    ...
    >
      ...
      <el-button type="primary" size="small" @click="downloadBatchFile" v-show="!isDelete" >{{$i18ns('下载')}}</el-button >
    </el-form>
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      this.cParentScope.parentFormModel.applid = this.cParentScope.parentFormModel.loanFileInfo.applid;
      this.cParentScope.parentFormModel.buctetKey = this.cParentScope.parentFormModel.loanFileInfo.buctetKey;
      this.cParentScope.parentFormModel.bucket = this.cParentScope.parentFormModel.loanFileInfo.bucket;
      this.cParentScope.parentFormModel.fileName = this.cParentScope.parentFormModel.loanFileInfo.fileName;
      this.cParentScope.parentFormModel.filePath = this.cParentScope.parentFormModel.loanFileInfo.filePath;
      ...
      this.buctetKey = this.cParentScope.parentFormModel.buctetKey;
      this.bucket = this.cParentScope.parentFormModel.bucket;
      this.fileName = this.cParentScope.parentFormModel.fileName;
      this.rules = {};
      let fileMap = {
        url: this.cParentScope.parentFormModel.loanFileInfo.filePath,
        name: this.cParentScope.parentFormModel.loanFileInfo.fileName,
      }
      this.cParentScope.parentFormModel[this.formKey] = JSON.stringify(fileMap);
      ...
      this.fileList.push({name: this.cParentScope.parentFormModel.fileName, status: "success"});
    },
    methods: {
      /**
       * 文件清单change回调
