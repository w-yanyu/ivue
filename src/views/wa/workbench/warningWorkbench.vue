...
export default {
  ...
  created() {
    ...
    this.getDictList("E_ROUTE_TYPE", "applstList"); // 获取申请状态数据字典
  },
  mounted() {
    ...
    this.c_$params.proc_inst_id = (this.cParentParams.proc_inst_id ? this.cParentParams.proc_inst_id : this.$dataBus.getCurrentcParams(this).data_proc_inst_id);
  ...
  methods: {
    ...
    topInpt() {
      ...
      this.ruleForm.custna = (this.cParentParams.custna ? this.cParentParams.custna : this.$dataBus.getCurrentcParams(this).data_custna);
      ...
      this.ruleForm.taskid = (this.cParentParams.cur_task_id ? this.cParentParams.cur_task_id : this.$dataBus.getCurrentcParams(this).data_cur_task_id);
    },
    /**
     * 获取可选节点列表
     */
    getRoadList() {
      let params = {
        ...
        tkinid: (this.cParentParams.cur_task_id ? this.cParentParams.cur_task_id :this.$dataBus.getCurrentcParams(this).data_cur_task_id),
    ...
    getTabInfo() {
      let params = {
        framecd: this.cParentParams.task_url1
          ? this.cParentParams.task_url1
          : this.$dataBus.getCurrentcParams(this).task_url1,
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          let params = {
            ...
            taskid: (this.cParentParams.cur_task_id ? this.cParentParams.cur_task_id : this.$dataBus.getCurrentcParams(this).data_cur_task_id),
        ...
        if (valid) {
          if (this.interForm.result) {
            let params = {
              ...
              proc_inst_id: this.interForm.proc_inst_id,
